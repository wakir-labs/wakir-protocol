...
This is the follow-on to (capability-policy
NATS-KV backend, CAS-pin, watch-stream). This module adds an
...
- T-CPP-REV-aux-lww-allows-unrevoke: This module LWW ``put`` does
  NOT enforce revocation-monotonicity (consistent with this module
This module LWW vs. CAS-pin safety-invariant policy).
"""
...
from wakir_protocol.schemas.capability_policy_nats_kv_backend import (
...
# ---------------------------------------------------------------------------
# Mock KV (CAS-aware, mirrors this module test fixture shape)
# ---------------------------------------------------------------------------
...
def test_t_cpp_rev_08_envelope_back_compat_without_revocation_keys():
    """T-CPP-REV-08: an envelope from (no
    ``revoked_at`` / ``revocation_reason`` keys) decodes byte-equally
    via the additive this module decoder to an unrevoked policy.
    """
    legacy_payload = {
        "schema": VALUE_SCHEMA,
        "registered_by": "wirelang-eng",
...
This module rationale: LWW writes are operator-deliberate, the
