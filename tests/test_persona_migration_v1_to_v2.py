...
"""``V1ToV2Step`` migration tests (V10-Migration-Pfad).

Coverage map (the design sketch §3 + §4 + §5)
========================================

This pack tests the converter-side V1-to-V2 migration step that the
This module sketch (``wirelang/specs/persona-schema-v10-migration-vorbereitung.md``)
specified as the implementation surface. An earlier increment authored the
``persona-v2`` JSON-Schema; This module lands the converter step itself.
...
2. **V1-to-V2 input-unchanged:** the input v9 dict is **not** mutated by
   the migration call (the design sketch §3 step rule 2 / this module
...
3. **V1-to-V2 source-version-strict:** a dict that already declares
   ...
   and returns the input unchanged. Mirrors this module's
...
5. **V0-to-V2 multi-step chain (M-1 direct anchor):** the v8 fixture,
   ...
   that the consensus marker companion memo flagged as
...
``wirelang/tests/fixtures/persona_definitions/``, created in this module
This module.
"""
...
def test_v0_to_v2_full_chain_pin_match() -> None:
    ...
    This is the **M-1 (linear-chain) direct-anchor** from the design sketch
    §3.2: a multi-step chain whose endpoints differ in two
    ``schema_version`` lifts. The consensus marker companion
    memo flagged this as currently indirect; This module lands the
    direct test vector.
    """
    migrated = migrate_persona(
        _v8_path(),
