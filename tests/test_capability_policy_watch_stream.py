...
This module (S5-5). Tests the additive watch-stream surface
...
This module LWW path (``put`` / ``get`` / ``snapshot`` / ``snapshot_registry``)
and the CAS-pin path (``put_with_revision`` /
``get_with_revision``) are unaffected; This module surfaces are purely
additive.

Pattern source: the schema-registry watch-stream in
...
- T-CPP-WS-09: a frozen registry from a watch-fed snapshot exposes
  ...
  path (this module cross-reference) and gates byte-identically.
...
# ---------------------------------------------------------------------------
# Mock KV with watch surface (mirrors this module schema-registry mocks)
# ---------------------------------------------------------------------------
...
def test_t_cpp_ws_09_live_snapshot_gates_consistent_with_full_snapshot():
    ...
    identically to the full-snapshot path against this module
