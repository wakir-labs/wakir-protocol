...
"""persona-v2 (V10) test-coverage extension.

Coverage map
==========================================================

An earlier increment authored ``wirelang/schemas/persona-v2.json`` (5 mandatory
...
three negatives). An earlier increment landed ``V1ToV2Step`` and
``test_persona_migration_v1_to_v2.py`` (6 tests). This module (this
file) extends both axes:

A. **Reserved-fields positive-path coverage with non-empty content** —
This module schema-tests only exercise the reserved fields as bare ``{}``
   maps. This module anchors each of the three reserved fields independently
...
C. **Full v2 field-union schema test (13 fields)** — every additive
   ...
   Total v2-schema-defined fields = 13. The task brief mentioned
...
D. **V0→V2 multi-step hardening (resolver associativity)** — V10-spec
   ...
   ``v8 → v1 → v2``. The earlier pack covers the single-call path
   and the M-1 direct-anchor; This module here adds the explicit pairwise
   equality test.
...
from wakir_protocol.persona import (
    compute_persona_hash_from_canonical,
    migrate_persona,
)
from wakir_protocol.persona._internal.migration_steps import (
    REGISTERED_STEPS,
    V0ToV1Step,
    V1ToV2Step,
)
from wakir_protocol.persona._internal.pin_pack_constants import (
    PERSONA_HASH_PIN_V8_MIGRATED_TO_V2,
    PERSONA_HASH_PIN_V9_MIGRATED_TO_V2,
)
from wakir_protocol.persona.persona_migration import _resolve_chain


SCHEMA_PATH = (
    Path(__file__).resolve().parent.parent / "wakir_protocol" / "schemas" / "persona-v2.json"
)
FIXTURE_DIR = (
...
This module pack stays self-contained (no cross-file fixture import)."""
    return {
        "name": "pengine",
        "description": "Persona-engine engineer (coverage fixture).",
        "tools": ["Read", "Bash"],
...
The earlier ``test_v2_reserved_fields_present_but_empty_validates``
    locked the bare ``{}`` case (Default-Lock A-2 / Option D-A drop-empty
    discipline). This module here locks the *non-empty* positive path so that
...
def test_v2_schema_accepts_full_field_union_simultaneously(
    ...
    3 reserved fields also carry non-empty payloads. This module here locks
    ...
    the task brief described this as "11 fields" (a top-level-only
...
def test_v0_to_v2_chain_equals_pairwise_composition() -> None:
    ...
    (``test_v0_to_v2_full_chain_pin_match``); This module here adds the
