...
Covers the ``pin`` subcommand declared in :mod:`wakir_protocol.persona.cli`.
The subcommand is a minimal-footprint shell-pipeline wrapper around
:func:`wakir_protocol.persona.persona_canonical_form.read_canonical_subset`
...
from wakir_protocol.persona._internal.pin_pack_constants import PERSONA_HASH_PIN_V9
...
pytestmark = pytest.mark.skipif(
    not RUST_FIXTURE_DIR.exists(),
    reason="Rust persona-cli fixtures live in wakir-runtime; cross-repo parity test runs in runtime CI only.",
)
...
def test_pin_v9_stdout_byte_identical_to_frozen_fixture(
    ...
    Byte-identity anchor in the V-907-anchor pin-pack (this module entry).
    """
    rc = main(["pin", str(V9_FIXTURE), "--quiet"])
    captured = capsys.readouterr()
    assert rc == 0
