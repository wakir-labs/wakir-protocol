...
Datalog caveat vocabulary. It supersedes the earlier non-normative sketch (removed from the
repository; the git history before tag `archive/pre-phase-4` retains
it) and turns its predicate
...
- The TV-W-2 pin-stability guarantee (§6): which producer changes
  ...
  and which may not.
- **v0.2.1 patch (ADR-0052, this revision 2026-05-12):** Class-P
...
| `caveat_hash(self_hash)` | **promoted to N1 in v0.2.1 (ADR-0052 approved 2026-05-12, landed this revision 2026-05-12)** | landed — see §3.4-N1-promoted-slot below and §6.7 |
...
5. **Test inventory (landed).** This revision adds five ratification
   probes in
   `wirelang/tests/test_caveat_hash_promotion_substrate.py`
   (T-CHP-07..11) on top of the seven this revision substrate probes
...
This revision) provides the substrate for cross-org
...
**v0.2.1 patch update (ADR-0052).** A second
...
This revision (S2-2) lands the live N2 evaluator for
...
This revision (S2-3) lands the production-target backend
for the V-908 federation-route registry as
`wirelang/federation/route_registry_nats_kv_backend.py`. This revision
...
This revision (S2-4) lands the N3 multi-FTD delegation-chain
walker in `wirelang/federation/n3_chain_walker.py`. N3 is the third
iteration after N1 (this revision stub: vocabulary reservation only) and
N2 (this revision single-hop live evaluator). N3 implements the
...
This revision (S2-5) extends the backend
(`wirelang/federation/route_registry_nats_kv_backend.py`) with a
watch-based incremental snapshot layer. The §5.6 implementation
...
synchronous-bridge contract makes the swap source-compatible." this revision
...
- Zone B (NATS-KV × Wirelang): the watch-stream consumes the DevOps track's
  Phase-1 NATS-JetStream substrate (orchestrator I-3 this revision compose
  + this revision bucket inventory). The `BUCKET_NAME`
  ...
  Z-B note may be delivered to the DevOps track's inbox documenting the
  expected `watchall()` surface; no Z-B marker is required
  because Phase-1b form ships under the existing this revision marker
...
Test coverage:
`wirelang/tests/test_federation_route_registry_nats_kv_backend.py`
— 26 tests green (13 this revision T-NKV-01..10 + 3 aux probes, plus 13
This revision T-NKV-WS-01..10 + 3 aux probes covering Shape-1 async-iter,
...
The drift envelope from (`production - sandbox =
144 ±5`) covers TV-W-2 once implemented. Adding the TV-W-2 test
module bumps `EXPECTED_DELTA` per protocol; the workflow-
...
ADR-0052 (decision date 2026-05-12, supervisory board ratified Option B
on CEO recommendation) promotes `caveat_hash(self_hash)` from
Class P (§3.4) to Class N1 via the v0.2.0 → v0.2.1 schema patch.
This subsection ratifies the verifier behaviour. Implementation
landed in (2026-05-12); test inventory is
T-CHP-01..11 in
...
The implementation does NOT shift the
...
+5 to the delta. This revision baseline `EXPECTED_DELTA=149` becomes 154
subsequent (drift 5 = tolerance 5, at-edge). The `tests.yml`
...
The Phase-2 sketch is **superseded** by this document and has been
removed from the repository (the git history before tag
`archive/pre-phase-4` retains it); verifiers and producers MUST cite this document
...
ratification time. TV-W-2 implementation in This revision adds the
