...
# Persona-Schema V10-Migration Vorbereitung (design sketch)

| Field | Value |
|---|---|
| Spec ID | V-907 / ADR-0036 (next-major-step preparation) |
| Phase | 1b this revision (sketch); implementation pending ceo-slot sprint-3 trigger |
...
- **Pin-pack vector numbering** (`v1..v9`, future `v10`, `v11`, ...).
  ...
  `wirelang/persona/_internal/pin_pack_constants.py`. This revision
...
| Pin-pack vector | Schema-version | Provenance |
|---|---|---|
| `v9` | `persona-v1` (current latest) | this revision baseline + this revision `V0ToV1Step` migration target |
...
constant name (frontend-slot this revision outbox); this
sketch keeps that constant stable and adds new ones.

## 1. Purpose

The V0-to-V1 self-migration step (this revision `V0ToV1Step`) was a
schema-version-only lift: rename the `schema_version` const, pass
everything else through. This revision prepares the *next* major
...
(possibly this revision or later) start coding without re-deriving
...
- **Reserved fields** — keys carved out in the schema with
  ...
  even though this revision leaves their meaning open.
...
|---|---|---|---|
| `model_pin` | object (optional) | absent | Carries an explicit model-version pin so a persona's behaviour is reproducible across model upgrades. Phase-2 alignment item; This revision records the hash-input shape so v2-inputs that supply it are byte-stable. |
...
allow-list with type `object` or `string` so that future this revision
...
| `container_bridge_spec` | kai | Zone-J persona-container-bridge override; This revision likely consumes it. |

Reservations are a low-cost forward-compat device: the schema
allows the key (so a v10 file with it is parseable), but this revision
This revision does not write semantics for them. Default-Lock A-2 is upheld
...
- **No field removal.** No top-level or nested key in `persona-v1` is
  removed in `persona-v2`. M-2 in the consensus marker aggregate
  (this revision companion memo) is the load-bearing assumption.
...
`source_version` and walks forward. The cycle-guard
(32) is sufficient.
...
is no native `persona-v2` fixture before this revision (where the
v10 fixture is authored).

### 3.1 Pin-pack expansion plan

This revision (when implementation lands) extends
...
# V1ToV2Step.apply on the v9 fixture.
...
Pin-bytes are computed at fixture-author time in a box;
...
This **is** the M-1 (linear-chain) direct-anchor that this revision
consensus marker companion memo flagged as currently *indirect*. A
...
The roundtrip pack (`test_persona_migration_roundtrip.py`,
9 tests) extends in with the following test classes. The
...
This revision (V10-Migration-Vorbereitung) is a **sketch only**, no
new tests authored. The implementation box (this revision) adds
~16 tests:
...
| Negative | 2 |
| **Total this revision delta** | **~16** |

Persona-cluster suite would move from 95 (this revision closeout)
to ~111. Full `wirelang` suite from 357 to ~373. (Numbers are
projection; this revision baseline is verified, projection
is sketch.)

## 5. Default-Lock A-2 re-verification (consensus marker handover)

The consensus marker aggregate (companion memo)
classified the four Default-Lock markers as:

| Marker | Statement | this revision status |
|---|---|---|
| **M-1** | Linear chain, no branching DAG | indirect (single-step this revision chain) |
| **M-2** | Additiv-only (no field removal) | direct (this revision V0-to-V1 step) |
| **M-3** | Migration is intentionally non-invertible | direct (this revision roundtrip pack §"M-3-direkt") |
...
V9-to-V10 / V0-V1-V2 implementation in this revision would convert:

| Marker | this revision status |
|---|---|
...
This is a substantial consensus marker hardening: three of four
markers move from indirect to direct in this revision. The ceo-
...
This sketch does **not** modify the consensus marker protocol file.
That edit is hr-slot domain. This revision implementation, when it
...
semantics in this revision. For pin-stability, the canonical-subset
...
There are two discipline options this revision may take:
...
This revision implementation box. Either way, the test-class
in §4.3 is the canary.

## 7. Cross-reference to frontend-slot consumption (V9-Pin sample)

The frontend-slot audit-trail-browser and persona-
inspector consume the V9-pin from
...
- **Per-persona V9/V10-Cohabitation.** A given persona-definition
  ...
  post-migration pin (i.e. always-current-target). The frontend-slot follow-up hook carries
  this decision.
- **TypeScript-Interface-Generator Pipeline (O3).** When the Persona-V9-Pin-Sample is exposed via the TS-
  generator pipeline (parallel to the wat-eng-slot TS-interface-
...
triggered by this sketch; This revision Auftakt is frontend-slot
domain.

## 8. Push-Bedingung mapping (this sketch)

Per this revision brief: "sketch + §9 clean → β-Push (Stack auf
this revision oder neuer Branch)."
...
1. **HR-slot ratification of `persona-v2` content.** This sketch
   ...
   (this revision consensus marker T-B trigger pattern).
2. **`V1ToV2Step` implementation + `REGISTERED_STEPS` extension.**
   ...
   **Status:** done — `V1ToV2Step` lands beside
   ...
   happened on this revision). The canonical-subset extractor's accepted-set
   ...
   direct-anchor test vector) collapse partially under this revision
...
5. **Pin-pack constant additions.** `PERSONA_HASH_PIN_V10`,
   ...
   **Status:** partially done — the two
   ...
   the aliased `PERSONA_HASH_PIN_V8_MIGRATED_TO_V2`) landed at this revision
...
9. **Frontend-slot this revision paired hand-off.** §7 cross-
   reference; frontend-slot domain pickup.
10. **consensus marker-Aggregat indirect-to-direct upgrade memo.**
    Once this revision This revision tests are green, file an hr-slot-side
    ...
    **Status:** M-1 direct-anchor *test vector*
    ...
    trigger memo to upgrade the consensus marker-Aggregat from
    indirect-anchor to direct-anchor remains pengine-Hand-pendent
    and will be drafted in a follow-up box (separate from
    code-landing).

## 10. §9-sweep self-verification (this file)

```
$ grep -nE "<clear-name-list>" \
...
- `wirelang/schemas/persona-v2.json` — proposed.
