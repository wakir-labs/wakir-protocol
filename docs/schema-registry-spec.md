...
— the Phase-2c live HTTPS counterpart to the v0.36.0 this revision
...
bridge-consumable :class:`FetchedTrustBundle`. Closes this revision
...
hermetic adapter used. Compatibility note: the hermetic
adapter remains byte-identical; both adapters now coexist as
explicit hermetic vs. live siblings.

**v0.36.0 — Multi-Org-Federation Live-Component
...
the v0.25.0 :class:`SpiffeCrossTrustDomainBridge` to the
This revision
...
changes; the v0.25.0 bridge surface remains byte-identical;
existing single-org callers are unaffected. New §5.21 (Live-
Component peer-trust-bundle-fetcher adapter).

**v0.35.0 — Operator-CLI + Live-Tail-Replicator detect_replay-
...
(`wirelang.cli.marker_stack_reduce`) wrapping the v0.30.0
in-memory reducer + v0.31.0 durable backend into one
...
callsite-mirror** on the v0.28.0
...
behaviour is byte-identical to this revision. v0.35.0 is an
**additive minor bump** per §3.2; no breaking changes; all
pre-existing Replicator constructions remain valid; this revision
This revision surfaces remain byte-identical. New §11 (Operator-
CLI annex) and §5.20 (Replicator-side detect_replay-callsite-
mirror).

**v0.34.0 — Durable Sequence-Number-Ledger + OTS-anchored
...
the v0.33.0 in-memory ledger to production-grade
...
breaking changes; the v0.33.0 surfaces remain byte-
identical; this revision
...
**v0.33.0 CaveatOverrideEvent Cross-Org-Export-Surface
anchor (2026-05-13).** This version adds the cross-org export
surface for the v0.30.0
`CaveatOverrideEvent`
(`wirelang.federation.caveat_override_export`). Pattern-mirror on
the v0.27.0 `UnrevokeAuditMarker` cross-org-export. New
load-bearing substantive additions over the pattern:
...
the v0.30.0/This revision surfaces remain byte-identical.

**v0.32.0 annex-note.** Version v0.32.0 is reserved for the
parallel the persona-engine track's Phase-2 `persona-engine-format-spec.md` bump
(this revision parallel-deliverable). The two spec axes are
orthogonal (persona-engine vs federation-export); v0.33.0 lives
under the additive-minor convention and explicitly does not
back-merge into v0.32. If the persona-engine v0.32.0 lands first, the
...
**v0.32.0 — Persona-Engine-Format integration anchor
...
**v1.2** (v1.0 landed first, v1.1 added the
HR counter-proposal-1 `synthesis_default_exceptions` table,
v1.2 added the §3.7 lifecycle protocols
...
Cross-Review-Zone-J (container-bridge with the DevOps track) **TRIGGERED** as
shape-only reservation (no DevOps-side Quadlet change yet).

**v0.31.0 Persistent Marker-Stack NATS-KV Backend anchor
(2026-05-13).** This version adds the persistent storage tier
for the v0.30.0 marker-composition engine
...
Cross-reference: the v0.29.0 and v0.30.0 change-log rows below
summarise the Cross-Org verifier and Multi-Marker composition
substance; the git history before tag `archive/pre-phase-4` holds
the per-increment detail.

The RealAdapter-Mirror
(`wirelang/adapters/real_nats_adapter/`) remains documented in
`identity-substrate.md` §5.8 (paired with
`real_spiffe_workload_api.py` v0.20.0 Slot-2-Mirror); the
...
the V-908 `RouteRegistry` Protocol shipped in a separate revision.

The registry is the natural consumer of the DevOps track's Phase-1 NATS-KV
inventory `wakir-schemas` bucket (`scripts/init-nats-buckets.py`,
This revision). v0.1.0 lands the
...
- Bucket-config drift-test against the DevOps track's `wakir-schemas` inventory
...
- The CAS-pin upsert path
  ...
  `get_with_revision`) — Phase-1c reclassification of this revision
...
- The publisher CLI (`wirelang/schemas/publisher_cli.py`) exposing a
  ...
  modes over the backends) and `dry-run` (validate
...
- The cross-bucket replication layer (`wirelang/schemas/replication.py`)
  ...
  composing this revision source-side watch-stream with / this revision
...
- The replicator is a *thin composition*: it adds no new method on
  ...
  the existing This revision surfaces.
- This revision covers two production scenarios: multi-org federation (one
...
- 12 hermetic determinism tests (T-SR-REP-01..12) over the same
  in-memory CAS-aware KV mock used by / this revision.
...
- ~~OI-7-Phase-1c-CAS~~ (CONSUMED in this revision).
- ~~OI-7-Phase-1c-watch~~ (CONSUMED in this revision).
- ~~OI-7-Phase-1c-publisher~~ (CONSUMED in this revision).
- ~~OI-7-Phase-1c-replication~~ (CONSUMED in this revision).

**v0.6.0 (this revision):**
...
This revision envelope codec (`_entry_to_envelope` / `_envelope_to_entry`)
  is unchanged in v0.6.0; the new `envelope_with_signature`
...
- The `kid` references a `public_keys` entry on the registering
  ...
  to a 32-byte Ed25519 public key. Phase-2-v0.6.0 does
...
- **VerifyMode** policy:
  ...
  - `STRICT` (Phase-2-end): entries without a `signature` slot
    ...
    is operator-controlled; v0.6.0 ships only the policy
    surface, not the activation switch.
- **`NatsKvSchemaRegistry` surface remains UNCHANGED** in this revision
This revision. Signing happens at envelope-build time: caller signs the
...
- **Cross-Review-Zone-1 (Identity-Substrate) TRIGGERED:** signing
  ...
  Identity-Substrate consumer touch-point. WAT-side
  ...
  the anchored byte-string). Cross-Review-Memo to the WAT track recorded
  in v0.6.0 outbox §2.

**v0.7.0 (this revision, additive over v0.6.0):**
...
- The kid → Ed25519 public-key resolver
  ...
  `KidResolverError`.
- The resolver lives in `wirelang.identity` (Wirelang-track-Default per
  Z-1-K-1: "resolver-layer in `wirelang.identity` separat";
...
- **Byte-accurate AIP-document field**: the resolver matches the
  ...
  as `kid`, not `id`). The Zone-1-annex consensus marker's
  Wirelang-track-Default phrasing "public_keys[i].id" refers to the same
  identifier slot; §5.9 captures the byte-accuracy note explicitly so
  future implementations cannot drift on the field name.
- **Curve-Choice filter** per Z-1-K-3 (Identity-Document-
...
- This slot closes **Z-1-K-1** (kid-Resolver-Shape) and
  **Z-1-K-3** (Curve-Choice reinforcement). Z-1-K-2
  (JCS-Resolver-Lock) and Z-1-K-4 (STRICT-Mode-Activation-
...
- **Boundary**: this slot does NOT fetch the AIP document over
  ...
  (verification stays caller-driven, consistent with v0.6.0
  Phase-2 boundary).

**v0.8.0 (this revision, additive over v0.7.0):**
...
- **Transport delegation**: HTTPS fetch is delegated wholesale to the
  Phase-1b V-908 `HTTPSDocumentTransport` (this revision PS-6 module); no
...
- This slot closes the v0.7.0 §5.9 boundary item
...
- **Cross-Review-Zone-1 (Identity-Substrate) non-touched**: the four
  ...
  `anchor_required` toggle is an orthogonal this revision hard-vs-soft policy,
  NOT the Z-1-K-4 STRICT-mode signing toggle. JCS
  canonicalisation reuses the Z-1-K-2 resolver-indirection
...
**v0.9.0 (this revision, additive over v0.8.0):**
...
- **Stateful tier on the stateless verifier**: the underlying
  `wirelang.identity.verify_aip_signature` (v0.6.0) is
...
- **Cache-key construction**: `SHA-256` over the byte-deterministic
  ...
  `wirelang.identity.aip_signing` (Z-1-K-2 JCS-Resolver-Lock
  is preserved; no new JCS path).
- **Cache-key body digest is byte-equal to the v0.8.0
  ...
  Production callers that have already fetched the body via this revision
...
- This slot consumes the **AIP-document signature-verification
  ...
  Identity-Substrate slot in §5.3; v0.9.0 ships it now to
...
- **Boundary**: this slot does NOT replace
  ...
  `HTTPSAipResolverCache` and the v0.8.0 transport-fetch
...
**v0.10.0 (this revision, additive over v0.9.0):**
...
This revision gate is an *additional* check that callers MUST run
...
- **`NatsKvSchemaRegistry` surface remains UNCHANGED**. No backend
  ...
  CLI-level slot, not this revision substance).
- **Cross-Review-Zone-1 (Identity-Substrate) non-touched**: the
  ...
  this revision-1 non-touched). The gate does not canonicalise anything
  (Z-1-K-2 JCS-Resolver-Lock non-touched). The gate is
  ...
  this revision-3 reinforced rather than touched). The gate is
  orthogonal to `wirelang.schemas.entry_signing.VerifyMode`
  (Z-1-K-4 non-touched; the STRICT-mode-activation-owner
...
- ~~AIP-document transport-fetch (`did:web` / `aip:web` HTTPS
  bridge)~~ (**CONSUMED in v0.8.0** by
  `wirelang.identity.aip_document_transport_fetch`; see §5.10).
- ~~AIP-document signature-verification cache tier for the
  transport-fetch layer~~ (**CONSUMED in v0.9.0** by
  `wirelang.identity.aip_signature_verification_cache`; see §5.11).
- STRICT-mode operator-activation toggle (Z-1-K-4 remains
  open — the resolver is policy-agnostic; the toggle question is
  a Phase-2-roadmap consensus decision).

The Phase-1c slots are all consumed: An earlier increment consumed
**OI-7-Phase-1c-CAS**, An earlier increment consumed **OI-7-Phase-1c-watch**, this revision
consumed **OI-7-Phase-1c-publisher**, This revision consumes
**OI-7-Phase-1c-replication**. v0.6.0 consumes
**OI-7-Phase-2-sig**; v0.7.0 closes the Zone-1-annex
follow-up `kid → public-key resolver`; v0.8.0 closes the
v0.7.0 §5.9 boundary item `AIP-document transport-fetch`;
v0.9.0 ships the AIP-document signature-verification cache
...
## 2. Bucket identity (cross-reference the DevOps track inventory)
...
**Cross-review memo (DevOps track):** the registry adds NO new bucket to the
Phase-1 inventory (the `wakir-schemas` bucket is already in the DevOps track's
4-bucket inventory). the DevOps track's 5th bucket (`wakir-federation-routes`,
This revision) is paired Z-B-update; v0.1.0 does NOT add a
6th bucket. The registry is a *consumer* of the existing inventory
slot; the only DevOps-side acknowledgement requested is a paired-update
note in the bucket-inventory cross-reference for 's
...
v0.21.0 ADR-0052 patch lifts to v0.2.1):
...
| `federation` | `datalog-caveat`              | `0.2.1`  | `datalog-caveat.json` (ADR-0052 Class-P promotion of `caveat_hash`, v0.21.0) |
...
The v0.1.0 backend module ships at
...
### 5.3 What the shipped revisions cover, and what Phase-2 still does NOT do
...
- Composition contract: source-side reads use this revision watch-stream
  surfaces (`open_watch_stream`, `WatchEvent`, `LiveSchemaSnapshot`
  via `snapshot()` for bootstrap); target-side writes use this revision LWW
...
- The `kid` references a `public_keys` entry on the registering
  ...
  public key is the caller's responsibility (v0.6.0 does
...
- Verification policy is governed by `VerifyMode`:
  ...
  - `STRICT` (Phase-2-end): entries without a `signature` slot
    raise `SchemaRegistrySignatureError`. Phase-2-end activation
    is operator-controlled; v0.6.0 ships the policy
...
**v0.7.0 lands (additive over v0.6.0):**
...
- The module is a pure composition over the Phase-1b V-908
  `HTTPSDocumentTransport` (this revision PS-6) and the V-908 §3.4 DNS-anchor
...
- Cross-Review-Zone-1 (Identity-Substrate) **non-touched**: the four
  Z-1-K consensus points remain byte-identical (the
  `anchor_required` toggle is an orthogonal this revision hard-vs-soft
  policy, NOT the Z-1-K-4 STRICT-mode signing toggle).

**v0.9.0 lands (additive over v0.8.0):**

- The AIP-document signature-verification cache tier
  (`wirelang.identity.aip_signature_verification_cache`: stateful
  in-process bounded LRU+TTL cache around the v0.6.0
...
**v0.10.0 lands (additive over v0.9.0):**
...
- `NatsKvSchemaRegistry` surface UNCHANGED; no new envelope, no
  ...
  v0.11.0**, §5.13: `wakir-schema-registry publish
...
- No CAS-quorum upserts on top of multi-replica clusters (this revision
...
- ~~No AIP-document transport-fetch (`did:web` / `aip:web` HTTPS
  ...
  slot.~~ (**CONSUMED in v0.8.0** by
  `wirelang.identity.aip_document_transport_fetch`; see §5.10.)
- ~~No AIP-document signature-verification cache tier (this revision
  transport-fetch is stateless; production callers compose with
  the V-908 `HTTPSAipResolverCache` or an outer cache tier).
  Phase-3 Identity-Substrate slot.~~ (**CONSUMED in v0.9.0**
...
- ~~No persistent capability-policy distribution (the v0.10.0
  ...
  materialises a v0.10.0 `CapabilityPolicyRegistry` directly
...
- No STRICT-mode activation toggle (Z-1-K-4 still open;
...
1. Envelope `schema_id ↔ schema_body.$id` (gate 1, §5.2 / this revision).
2. Envelope `schema_body_sha256` ↔ recomputed JCS-anchored hash
   (gate 2, §5.2 / this revision).
3. Triple-derived key matches the entry (gate 3, §5.2 / this revision).
...
**Determinism contract (this revision invariant):**
...
long-running supervisors. It mirrors the V-908 this revision watch-stream
...
- Watch-stream resume-from-revision is a Phase-2 concern; this revision
...
into the existing this revision backend gates.
...
- This revision gates 1-3 (`schema_id ↔ $id`, body-hash match, key↔triple
     match) → VALIDATION_ERROR (exit 4).
- This revision CAS-pin call (only for `cas` and `create-only` modes) →
...
This revision watch surface. Operators who want post-publish observability
...
composition of three Phase-1c surfaces (this revision LWW, this revision CAS-pin,
This revision watch-stream); the replicator itself adds no new method on
...
- **Target side (this revision CAS-pin or LWW):** writes go through
...
**Determinism contract (this revision invariants):**
...
- This revision is **one-way**: source → target. Bidirectional replication
  with conflict-free CRDT-style merges is `OI-7-Phase-2-bidir-replication`
  reserved.
- This revision is **single-source / single-target** per replicator. Multi-source
  fan-in is achieved by running multiple `SchemaReplicator` instances
  against one target backend.
- This revision has **no resume-from-revision policy**: a connection drop
  forces a full bootstrap-and-tail restart. Resume policies are
  `OI-7-Phase-2-resume` reserved.
- This revision has **no envelope-side capability-token enforcement**; the
...
### 5.8 Entry-signing operational contract (v0.6.0)

The entry-signing layer is the canonical Ed25519 signature substrate
for schema-registry entries. It is a thin composition over this revision
...
A `SignedSchemaRegistryEntry` is the in-memory pair of this revision
...
- `STRICT`: an unsigned entry raises `SchemaRegistrySignatureError`
  ...
  operator-controlled (out of scope for v0.6.0).
...
**Determinism contract (v0.6.0 invariants):**
...
- The signing primitive is byte-identical to
  ...
  consistency.
- The `kid` field is a free-form string in v0.6.0; binding
  it to an AIP-document `public_keys` entry is the kid → key
  resolver's responsibility (out of scope for this slot). Cross-
  Review-Memo to WAT-side) is recorded in v0.6.0
  outbox §2.

**v0.6.0 boundary:**

- v0.6.0 ships the signing primitive and verify-mode policy;
  it does NOT ship the kid → key resolver.
- v0.6.0 does NOT plumb signature verification into the
  backend read path; verification is caller-driven.
- v0.6.0 ships the `STRICT` mode policy surface but NOT
...
- ~~Capability-token gating on `registered_by` (mapping `kid` to
  ...
  additive authorisation tier that runs alongside this revision
...
**Forward reference (v0.7.0):** the `kid` referenced in
...
### 5.9 kid-resolver operational contract (v0.7.0)
...
Z-1-K-1 consensus marker default ("resolver-layer in
...
The Wirelang-track-Default phrasing in the Zone-1-annex consensus marker
...
3. **Algorithm filter (Z-1-K-3 reinforcement)**: the
...
**Determinism contract (v0.7.0 invariants):**
...
- **Z-1-K-1 (kid-Resolver-Shape) CLOSED** by this module.
- **Z-1-K-3 (Curve-Choice for Schema-Registry-Sigs)
  reinforced**: `alg == "Ed25519"` filter enforced. The resolver
  cannot bridge from a secp256k1 entry to the Identity-Document
  signing layer.
- **Z-1-K-2 (JCS-Resolver-Lock) non-touched**: the
...
- The resolver does NOT plumb into the schema-registry backend read
  path. Verification stays caller-driven (consistent with this revision
This revision entry-signing boundary).
...
v0.7.0 test T-KID-RES-03 exercises it end-to-end against the
RFC-8032 Ed25519 test-vector seeds.

### 5.10 AIP-document transport-fetch operational contract (v0.8.0)
...
already obtained the AIP document, but does not specify how. this revision
This revision ships that step as a pure composition of two existing Phase-1b
...
`HTTPSDocumentTransport` (this revision PS-6 module). The layer:
...
The result is that This revision is byte-thin over the existing transport:
...
The `anchor_required` toggle is **orthogonal to** the v0.6.0
`VerifyMode.STRICT` signature-policy toggle (Z-1-K-4) — it
...
**Determinism contract (v0.8.0 invariants):**
...
- **Z-1-K-1 (kid-Resolver-Shape)** non-touched; this revision
  An earlier increment closed it. This revision feeds the resolver, does not modify it.
...
- **Z-1-K-3 (Curve-Choice = Ed25519)** non-touched;
  transport-fetch is curve-agnostic (it fetches a document, not a
  key).
- **Z-1-K-4 (STRICT-Mode-Activation-Owner)** non-touched;
  `anchor_required` is a local hard-vs-soft toggle and is
...
v0.8.0 test `T-AIP-FT-11` exercises steps 1-2 against the
RFC-8032 Ed25519 test-vector seeds; the resolver feed-through to
`verify_entry_signature` is covered by v0.7.0 test
`T-KID-RES-03`. The two tests together pin the end-to-end Phase-2
verifier pattern.

### 5.11 AIP-document signature-verification cache tier (v0.9.0)

The verification-cache tier is a stateful, bounded LRU+TTL cache on
top of the v0.6.0 AIP-document signing primitive
(`wirelang.identity.verify_aip_signature`). The verifier is
...
1. `SHA-256(JCS(body without document_signature))` hex of the
   AIP-document body. Identical in shape to the v0.8.0
   `jcs_sha256_hex` byte-anchor; reuses the same resolver-indirected
   `_jcs_canonicalize` from `aip_signing` (Z-1-K-2 lock
   preserved).
...
**Determinism contract (v0.9.0 invariants):**
...
- Cache transport-fetch outputs. The v0.8.0
...
- **Z-1-K-2 (JCS-Resolver-Lock)** non-touched; this module
  ...
  byte-equal to the v0.8.0 `_jcs_anchor_hex` shape.
- **Z-1-K-3 (Curve-Choice = Ed25519)** non-touched; the
...
- **Z-1-K-4 (STRICT-Mode-Activation-Owner)** non-touched;
  ...
  the verifier; the caller's STRICT-mode policy (v0.6.0
...
#    (v0.9.0, this section). Repeated calls with byte-equal
...
v0.9.0 test `T-AIP-SVC-02` pins the hit-on-second-verify
...
### 5.12 `registered_by`-capability-gating operational contract (v0.10.0)
...
The gate is *additive* policy on top of the v0.6.0
...
3. **`allowed_triples`** is a non-empty tuple of
   ...
   grammar (`*` / `?` / `[seq]`). the earlier increment's name-glob choice is
...
4. **`not_before` / `not_after`** define a validity window for the
   ...
   (consistent with the v0.7.0 `resolve_kid` `as_of`
   contract).
...
**Determinism contract (v0.10.0 invariants):**
...
2. **Caller-supplied `kid`:** the gate consumes
   ...
   It does NOT call the v0.7.0 `kid_resolver`; the kid →
...
- ~~This module does NOT distribute or persist policies. The
  ...
  Phase-3 slot.~~ (**CONSUMED in v0.12.0** —
  ...
  bucket; see §5.14. The v0.10.0 in-process
  `CapabilityPolicyRegistry` remains the evaluation surface;
  v0.12.0 ships the source-of-truth substrate that
...
- Z-1-K-1 (kid-Resolver-Shape) is consumed *upstream* by
  the caller; This revision takes the `signature_block["kid"]` byte-equal
...
- Z-1-K-2 (JCS-Resolver-Lock) is non-touched: This revision does
  not canonicalise anything; it is policy-evaluation only.
- Z-1-K-3 (Curve-Choice = Ed25519) is reinforced
  indirectly: This revision operates on the entry-signing layer which is
  Ed25519, but This revision carries no curve choice of its own.
- Z-1-K-4 (STRICT-Mode-Activation-Owner) is non-touched:
  ...
  may also run this revision capability-gating; the two checks compose
  independently.
...
# 2. Cryptographic verification (v0.6.0 §5.8).
ok = verify_entry_signature(
    signed, my_pub_key, mode=VerifyMode.STRICT,
)
assert ok, "signature does not verify"

# 3. Capability gating (v0.10.0, this section).
...
v0.10.0 test `T-RBC-02` pins the matching-policy allow with
...
#### 5.12.1 Revocation operational contract (v0.16.0, additive over v0.10.0 and v0.12.0–v0.15.0)

v0.16.0 adds an *explicit revocation* axis to
`CapabilityPolicy` that is distinct from the v0.10.0
...
- `disabled=True` is a **reversible operator-side state** ("this
  ...
  v0.10.0 spec wording is intentionally non-committal.
...
remains the single value schema; This revision adds two optional fields:
...
The decoder treats missing keys as `None` so this revision
...
**Gate-precedence amendment.** The v0.10.0 fallback ordering
(`POLICY_DISABLED` → `KID_NOT_ALLOWED` → `TRIPLE_NOT_ALLOWED` →
...
2. Otherwise fall through to the v0.10.0 disabled / kid /
...
pre-CAS `Gate 3` check (Gates 1 and 2 are the v0.14.0
...
**LWW non-enforcement.** The v0.12.0 `put` (LWW) path does
NOT enforce revocation-monotonicity. This is consistent with the
v0.14.0 rationale: LWW writes are operator-deliberate and
...
**Watch-stream surface (v0.15.0 unchanged).** A revocation
...
The four Z-1-K consensus points (kid-resolver shape,
JCS-resolver lock, curve choice Ed25519, STRICT-mode activation
owner) remain byte-identical.

**Cross-Review-Zone-B (the DevOps track NATS-KV) non-touched.** The
...
**Boundary: v0.16.0 does NOT ship (explicit).**
...
2. Watch-stream consumer-side revocation-event filter helper — the
   ...
   revoked_at != None`); a dedicated helper is this revision
   candidate.
3. Cross-bucket revocation replication (extend v0.5.0
   replication to carry revoked policies byte-precisely) —
This revision candidate.
4. Full Biscuit v3 binary-token revocation-list interpretation —
   Phase-3 substrate; lives in the Datalog evaluation layer per
   `wirelang/schemas/layer-3-capability-token.json`. this revision
...
### 5.13 Publisher-CLI capability integration (v0.11.0)

v0.11.0 lifts the v0.10.0 capability gate into
the publisher-CLI operator surface. The integration is **additive
over v0.10.0**: the v0.6.0 `sign_entry` primitive and
the v0.10.0 `gate_signed_entry` composition helper are wired
...
The v0.11.0 matrix adds **one** code; pre-existing codes are
byte-unchanged:
...
The v0.11.0 receipt adds three optional fields; pre-existing
fields are byte-unchanged. Pre-this revision callers that omit the
...
The v0.11.0 flow is the operator-CLI projection of the
v0.10.0 §5.12 Composition Pattern, with three differences
...
1. The verifier (`verify_entry_signature`) is NOT invoked on the
   ...
   consumer-side concern (v0.6.0 §5.8 + §5.11 caching
   tier).

2. The capability registry is loaded from disk every CLI invocation
   (no caching across runs). This is a deliberate v0.11.0
...
3. The signature block is **not** written to the bucket. this revision
This revision publishes the underlying `SchemaRegistryEntry` only; the
   ...
   neither shipped in v0.11.0).

#### v0.11.0 boundary (what this slot does NOT do)
...
- Does NOT push the signature block onto the `wakir-schemas` bucket.
  ...
  the v0.6.0 / this revision stack and lives outside
  the publisher CLI.
...
- ~~Does NOT distribute capability policies. The registry is loaded
  ...
  reservation per §5.12 / §7.~~ (**CONSUMED in v0.12.0** —
  `wirelang.schemas.capability_policy_nats_kv_backend` ships the
  `wakir-capability-policies` bucket and the
  `NatsKvCapabilityPolicyBackend` surface; see §5.14. v0.11.0
  publisher CLI is byte-unchanged: the `--capability-registry` flag
  still reads operator-local JSON. ~~A future `--capability-bucket`
  publisher-CLI flag that reads from the v0.12.0 bucket is a
This revision candidate; v0.12.0 is the substrate, not
  the CLI integration.~~ — **CONSUMED in v0.13.0**: the
...
v0.11.0 test `T-SR-PUB-CG-01` pins the loader round-trip;
...
#### Bucket policy source (v0.13.0, additive over v0.11.0)

v0.13.0 closes the operator-experience gap left by §5.14: the
publisher CLI gains the `--capability-bucket` flag (mutually
exclusive with `--capability-registry`) so a sign+gate publish can
draw its policies from the v0.12.0 persistent bucket
...
connect / cleanup contract from this revision. A poisoned bucket
...
- ~~No watch-stream on the capability-policy bucket (full-snapshot
  only; Phase-3 slot).~~ (**CONSUMED in v0.15.0** — the
...
- No `--capability-bucket` Biscuit-v3-binary-token interpretation
  (the bucket envelope is the v0.12.0 JSON shape; promotion
...
- No mutation of `NatsKvCapabilityPolicyBackend` (v0.12.0 is
...
- No new bucket on the orchestrator inventory (Z-B non-touched;
  `wakir-capability-policies` is the v0.12.0 bucket consumed
  as-is).
...
The v0.13.0 test inventory `T-SR-PUB-CB-01..10` (see §6.10
...
### 5.14 Capability-policy persistent distribution (v0.12.0)

v0.12.0 ships
`wirelang.schemas.capability_policy_nats_kv_backend`, the
persistent-distribution tier for the v0.10.0 in-process
...
The module exports (v0.12.0 surface):

- `BUCKET_NAME = "wakir-capability-policies"` — the new 7th bucket
  on the DevOps track's `PHASE_1_BUCKETS` inventory (Z-B paired-update pending).
- `BUCKET_CONFIG` — the mirror constant for the orchestrator-side
  ...
  (Phase-2)"`. Drift-policy is identical to the v0.1.0
...
- `CapabilityPolicyRecord` — a frozen dataclass wrapping this revision
This revision `CapabilityPolicy` plus operator-side bookkeeping
...
`registered_by` axis matches the v0.10.0 `CapabilityPolicy`
...
1. The record's embedded `CapabilityPolicy` bundle has already
   passed `CapabilityPolicy.__post_init__` (v0.10.0
...
The envelope decoder round-trips through the v0.10.0
`CapabilityPolicy` constructor so every v0.10.0 invariant is
re-enforced at decode time. Malformed envelopes surface as
`CapabilityPolicyEnvelopeError` (the persistent layer wraps the
v0.10.0 `RegisteredByCapabilityError` so consumers can catch
...
sorted-key lists (v0.1.0 determinism analogue).
...
gate evaluates them (v0.10.0 semantics: a disabled policy
...
- This module ships the persistent-distribution tier. It does NOT
  replace the v0.10.0 in-process registry: operators who
  prefer the v0.11.0 `--capability-registry` JSON-file path
...
- ~~This module does NOT auto-distribute policies to publisher-side
  ...
  (analogous to the schema-registry watch-stream, v0.3.0).
  The v0.12.0 slot is full-snapshot only.~~ (**CONSUMED
  in v0.15.0** — the live tail
  `NatsKvCapabilityPolicyBackend.watch()` plus the
  `LiveCapabilityPolicySnapshot` consumer surface land here as the
  pattern-mirror on the schema-registry watch-stream from this revision
This revision. The full `snapshot_registry` path remains supported and
  ...
  *consumer* surface; the synchronous gate
  ...
  operational contract (v0.15.0, additive over v0.14.0)**
...
- ~~This module does NOT ship a CAS-pinned upsert path. Policies are
  ...
  v0.2.0). v0.12.0 ships PUT (LWW) only.~~ (**CONSUMED
  in v0.14.0** — the CAS-pin path `put_with_revision` is now
  the opt-in lost-update-protection surface, byte-mirroring the
  v0.2.0 schema-registry CAS-pin contract; see the
  "CAS-pin operational contract (v0.14.0)" subsection below.
  The v0.12.0 LWW `put` path remains supported and orthogonal
  to the CAS-pin path.)
- ~~This module does NOT modify the v0.11.0 publisher CLI. A
  future `--capability-bucket` flag that reads policies from this
  bucket is a candidate; v0.12.0 is the
  substrate, not the CLI integration.~~ (**CONSUMED in this revision
  this revision** — the `--capability-bucket` flag now reads policies from
  ...
  end-to-end; see §5.13's "Bucket policy source (v0.13.0)"
  subsection. v0.12.0 substrate is consumed byte-unchanged.)
...
- **Z-1-K-1 (kid-Resolver-Shape):** v0.12.0 does
  NOT import `kid_resolver`. The `allowed_kids` field on a
  persistent policy carries kid strings byte-equal to the in-process
  v0.10.0 bundle; resolver chain is upstream of the gate
  call.
- **Z-1-K-2 (JCS-Resolver-Lock):** v0.12.0 does
  ...
  to `schema_body_sha256`). The v0.6.0 entry-signing path
  and its JCS canonicalisation are byte-unchanged.
- **Z-1-K-3 (Curve-Choice = Ed25519):** v0.12.0 is
  curve-agnostic. The persistent policy carries `allowed_kids`
  strings without a curve-axis; the v0.7.0 kid-resolver and
  v0.6.0 entry-signing primitives remain the curve-binding
  layer.
- **Z-1-K-4 (STRICT-Mode-Activation-Owner):** this revision
  This revision is orthogonal to `VerifyMode`. The persistent-policy layer
  ships persistence, not signature verification.

#### Cross-Review-Zone-B paired-update (DevOps-side action)

v0.12.0 IS a Zone-B trigger. The orchestrator-side
`PHASE_1_BUCKETS` inventory currently lists 6 slots
(post-v0.9.0):

| Slot | Name                              | Status                                    |
|------|-----------------------------------|-------------------------------------------|
| 0    | `wakir-schemas`                   | Live (v0.1.0 consumer)            |
| 1    | `wakir-aip-cache`                 | Live (AIP resolver)                       |
| 2    | `wakir-ftd-cache`                 | Live (FTD resolver)                       |
| 3    | `wakir-ftd-poisoned`              | Live (FTD poison-marker)                  |
| 4    | `wakir-schema-registry-entries`   | Phase-2 reservation (v0.8.0)      |
| 5    | `wakir-federation-routes`         | Live (V-908 consumer)                     |

v0.12.0 requests a 7th slot (`wakir-capability-policies`)
...
`replicas` on DevOps-side acceptance (this revision DevOps-side slot).
...
#### CAS-pin operational contract (v0.14.0, additive over v0.12.0)

v0.14.0 adds an opt-in lost-update-protection surface to the
persistent capability-policy backend. The substance is a
pattern-mirror on the v0.2.0 schema-registry
...
**Why CAS-pin on capability-policy authorship.** The v0.12.0
...
v0.2.0 schema-registry CAS-pin contract:
...
**Compatibility statement (v0.14.0 boundary).**

- v0.12.0 LWW surface (`put` / `get` / `get_by_pair` /
  `delete` / `list_keys` / `snapshot` / `snapshot_registry`) is
  byte-unchanged. T-CPP-01..10 from §6.11 remain green.
- v0.12.0 envelope (`wakir.wirelang.capability-policy-entry/1`
  on `wakir-capability-policies`) is byte-unchanged. M-2 conformance
  preserved.
- v0.12.0 bucket configuration (`BUCKET_CONFIG`) is
  ...
  paired-update memo.
- The v0.2.0 schema-registry CAS-pin surface
  ...
  v0.14.0 capability-policy CAS-pin surface is a parallel
...
- A Biscuit v3 binary-token interpretation of the policy envelope —
  ...
  v0.12.0 byte-unchanged.
- A mutation of `NatsKvSchemaRegistry` — the schema-registry
  backend is byte-unchanged.

#### Watch-stream operational contract (v0.15.0, additive over v0.14.0)

v0.14.0 shipped the CAS-pin write path on the
`wakir-capability-policies` bucket. v0.15.0 adds the live-tail
*read* path, mirroring the v0.3.0 schema-registry
...
**Why a watch-stream on capability-policies?** The v0.12.0 full
...
watcher shapes (byte-equal to the v0.3.0 schema-registry
contract):
...
`NatsKvCapabilityPolicyBackend.snapshot`. v0.15.0 does
...
- A CAS-pin write (v0.14.0 `put_with_revision`) yields one PUT
  event on the stream identical to a LWW PUT.
- A LWW write (v0.12.0 `put`) yields one PUT event.
...
v0.10.0 `CapabilityPolicyRegistry` is indexed by
...
#### Boundary: v0.15.0 does NOT ship (explicit)

- Watch-stream resumption / replay-from-revision
  (`watchall(..., resume_from=...)`) — Phase-3 slot. nats-py
  supports it; the v0.15.0 wrapper exposes the underlying
...
- Publisher-CLI composition of the live-tail consumer (a
  ...
  `wakir-publisher`) — candidate. This revision
This revision `--capability-bucket` flag remains a one-shot snapshot read
...
- ~~A `Replicator`-style cross-bucket capability-policy replication
  (analogous to v0.5.0 schema-registry replication) —
  ...
  v0.19.0 change-log entry. The v0.19.0 layer carries the
  v0.16.0 revocation-monotonic invariant byte-precisely
  across the cross-bucket boundary (a feature with no
  schema-registry-side analogue).
- A Biscuit v3 binary-token interpretation of the policy envelope —
  Phase-3 substrate; the in-bucket JSON envelope shape is
  v0.12.0 / this revision byte-unchanged.
- A mutation of `NatsKvSchemaRegistry` — the schema-registry
  backend is byte-unchanged.

### 5.15 Multi-Marker-Policy-Composition (v0.30.0)

**Status:** This revision lands the deterministic Multi-Marker-Policy-
Composition reducer in
`wirelang/federation/marker_composition.py`. this revision
This revision fixes the per-capability-token lifecycle reduction surface
...
- **`RevokeEvent(event_at, revocation_reason, tie_break, wat_anchor_manifest_id)`**:
  mirrors v0.16.0 `policy.revoked_at` set; transitions
...
- **`UnrevokeEvent(event_at, previous_revoked_at, unrevoke_reason, tie_break, wat_anchor_manifest_id)`**:
  mirrors v0.22.0 `UnrevokeAuditMarker` on the composition
...
- **`BridgeRevokedEvent(marker: BridgeRevocationMarker, tie_break, wat_anchor_manifest_id)`**:
  wraps the v0.29.0 `BridgeRevocationMarker`. Bridge
...
- **Zone 1 (Identity-Substrate):** `BridgeRevocationMarker` is
  ...
  `wirelang.federation.cross_org_attenuation_verifier` (this revision
This revision). The composition module does NOT duplicate the schema.
...
- **Zone 3 (OTS-Schema-Anker):** reserved for a future this revision
  schema-registry entry (four event-marker schemas + verdict
  envelope). Not opened in this revision.

#### 5.15.6 What v0.30.0 explicitly does NOT do

- **No persistent marker-stack bucket.** The reducer is a pure
  in-memory engine; persistence is this revision candidate
  (pattern-mirror on v0.12.0 capability-policy backend).
  **CONSUMED in v0.31.0** — see §5.16.
- **No marker-stack cross-org-export-surface.** Mirror on
  v0.27.0 `UnrevokeAuditMarker` export is this revision
  candidate.
- **No operator-CLI surface.** A `reduce-marker-stack` CLI
  subcommand pretty-printing the audit-trace is this revision
  candidate.
...
### 5.16 Persistent Marker-Stack NATS-KV Backend (v0.31.0)

**Status:** This revision lands the persistent storage tier for the
v0.30.0 marker-composition engine in
`wirelang/federation/marker_stack_kv.py`. Per-org append-only
NATS-KV buckets persist individual marker events; the live
`MarkerStack` is re-constituted on demand for the reducer
to yield a byte-identical `CompositionVerdict`. this revision
This revision fixes the on-the-wire envelope, the bucket-name family,
...
The v0.30.0 reducer is sort-stable: out-of-order
...
2. `get_marker_stack` reads all events for the token-id and
   hands them to the v0.30.0 `reduce_marker_stack`
...
This revision) and re-bootstrap.
...
`replicas=1`. Drift-policy: identical to the v0.12.0
...
five v0.30.0 event dataclass families).
...
Cross-org reads MUST flow through the explicit this revision
...
- Decode failures raise `MarkerStackEnvelopeError` and
  terminate the iterator (consistent with the v0.15.0
...
v0.15.0 watch-stream-handle adapter.
...
#### 5.16.10 What v0.31.0 explicitly does NOT do

- **No cross-org-export-surface.** Cross-org export of
  marker-stacks (mirror on v0.27.0 `UnrevokeAuditMarker`
  export) is this revision candidate.
- **No operator-CLI surface.** A `marker-stack inspect` /
  `marker-stack append` / `marker-stack watch` CLI subcommand
  is this revision candidate.
...
- **No mutation of the v0.30.0 reducer.** The reducer
...
sandbox-vs-host separation policy). All v0.31.0 tests
run against an in-memory mock that mirrors the v0.12.0
...
### 5.17 CaveatOverrideEvent Cross-Org-Export-Surface (v0.33.0)

v0.33.0 ships the cross-org export surface for the
v0.30.0 `CaveatOverrideEvent` so verifier-observed
issuer-side caveat-narrowing events propagate across the
This revision multi-org-federation substrate without leaking
operator-supplied free-form text or the raw caveat-chains
themselves. Pattern-mirror on v0.27.0
...
The exporter applies the same pattern as v0.27.0
...
ledger. v0.33.0 ships only `InMemorySequenceNumberLedger`
(hermetic-test-grade and single-process-exporter-grade); a
durable NATS-KV-backed implementation lives in this revision
...
- **No durable ledger.** `InMemorySequenceNumberLedger` is
  ...
  NATS-KV-backed implementation is this revision slot.
- **No OTS-anchored schema-registry entry** for the
...
- **No mutation of the v0.30.0 `CaveatOverrideEvent`
...
- **No mutation of the v0.27.0 `UnrevokeAuditMarker`
...
- **No live NATS interaction.** Live NATS connections are
  ...
  policy). All v0.33.0 tests are hermetic.
- **No replay-protection on the multi-org-attestation
  live-tail replicator** (v0.28.0). The replicator
...
Live NATS connections are operator-hand. All v0.33.0
...
This revision ships the durable tier for the v0.33.0
...
deployments install the durable tier without changing this revision
exporter surface.
...
permitted-character regex mirrors the `peer_org`
...
they are reproducible byte-for-byte per caller (v0.12.0 /
v0.31.0 envelope contract).
...
This is byte-equivalent to this revision
...
Cross-org reads MUST go through the explicit this revision cross-
...
`SyncSequenceNumberLedgerAdapter` implements this revision
...
- **No multi-org-attestation live-tail-replicator integration.**
  The v0.28.0 replicator currently has no per-payload
  replay-protection; a `detect_replay`-callsite-mirror in the
  replicator is this revision candidate.

#### 5.18.9 Sandbox boundary

Live NATS connections are operator-hand. All this revision
durable-ledger tests are hermetic and run against an in-memory
mock that mirrors the v0.31.0 `_MockKv` shape with an
...
Bitcoin-anchor submission is **operator-hand**: this revision
This revision sandbox-default `anchor_state` is `PRE_SUBMISSION`. The
...
#### 5.19.5 Was §5.19 explicitly does NOT do
...
### 5.20 Replicator-side detect_replay-callsite-mirror (v0.35.0 Part B)

This revision Teil B extends the v0.28.0
...
extension is: pre-existing replicator
...
- `PASS_THROUGH`: the replicator routes the event through the
  conflict policy as in (no behavioural change).
...
A production detector that wires the v0.33.0
`detect_replay` symmetric gate over a downstream
`SequenceNumberLedger` (this revision durable variant) is a thin
adapter:
...
`MultiOrgAttestationWatchEvent` into this revision
`ExportedCaveatOverrideEvent` when the cross-org channel carries
override-event payloads. This revision Teil B does NOT ship the
...
Live NATS replication is operator-hand. All this revision Teil B
detect_replay-mirror tests are hermetic and run against the
This revision `_MockKv` shape extended with the mock watcher surface.
No live NATS connection is established from the sandbox.

### 5.21 Multi-Org-Federation Live-Component peer-trust-bundle-fetcher adapter

This revision lands the **live-component** adapter that
wires the v0.25.0
:class:`SpiffeCrossTrustDomainBridge` to this revision
`infra/spire/federation/bin/spire_fed_bundle` hermetic-mode CLI.
...
The adapter is the integration anchor for this revision
Multi-Org-Federation substrate end-to-end:

- This revision :class:`MultiOrgRouteAttestation` envelope carries the
  `peer_trust_bundle_url`. The bridge passes it to the adapter.
- This revision :class:`NatsKvMultiOrgAttestationRegistry` stores the
  attestation durably; the bridge consumes a snapshot.
- This revision :class:`SpiffeCrossTrustDomainBridge` orchestrates the
  resolution. The adapter is its `trust_bundle_fetcher` slot.
- This revision :class:`CapabilityAttenuationChainVerifier` verifies the
  capability-chain riding the bridged route; the chain hash is the
  WAT-Audit-Federation-Annex anchor (WAT-track D-1 follow-up).
- This revision :class:`MultiOrgAttestationReplicator` replicates Org-A
  writes into Org-B's bucket; the bridge resolves on the target
  side post-replication.

The E2E roundtrip test
...
- v0.25.0 bridge is consumed via the *existing*
  `PeerTrustBundleFetcher` Protocol — no Protocol change.
- This revision modules are byte-identical to v0.35.0.
...
### 5.22 Multi-Org-Federation Phase-2c Live-HTTPS counterpart adapter

This revision lands the **Phase-2c live HTTPS counterpart**
to the hermetic
...
1. Reject if `trust_domain` is not in the pin set
   (`UnpinnedTrustDomainError`, re-used from this revision).
2. Reject if `url` does not equal the pinned URL for the requested
...
1. **Production**: `verify_ca_bundle_path=<path-to-pem>` — the TLS
   ...
   production federation. The CA-bootstrap lifecycle is the DevOps track's
...
This decision closes the **This revision Open-Item** (see
`2026-05-14-reza-sprint-7-pfad-b-tag-5-federation-live-component.md`
cross-review Zone-A D-2 the DevOps track). The hermetic adapter's
...
and MUST be root-path. The hermetic suffix convention is
...
config. The DevOps-side configs do NOT need a path-prefix rewrite.

#### 5.22.6 Sandbox boundary (ADR-0051)

The live module **is** the explicit boundary-crossing surface in
the Phase-2 federation substrate. The hermetic module
...
The live fetcher slots into the same this revision
Multi-Org-Federation substrate as the hermetic adapter,
...
- This revision module surfaces are byte-identical to
  v0.36.0.
...
v0.1.0 ships hermetic tests at
...
### 6.1 CAS-pin tests (v0.2.0)

v0.2.0 ships hermetic CAS-pin tests at
...
Total this revision test additions: 10 primary CAS-pin tests + 2 auxiliary
probes = 12.

### 6.2 Watch-stream tests (v0.3.0)

v0.3.0 ships hermetic watch-stream tests at
...
Total this revision test additions: 10 primary watch-stream tests + 2
auxiliary probes = 12.

### 6.3 Publisher CLI tests (v0.4.0)

v0.4.0 ships hermetic publisher-CLI tests at
...
Total this revision test additions: 12 hermetic determinism tests
...
### 6.4 Replication tests (v0.5.0)

v0.5.0 ships hermetic replication tests at
...
used by / this revision, with a wrapper that simulates the
read-then-mutate race window for the CAS-conflict path.

### 6.5 Entry-signing tests (v0.6.0)

v0.6.0 ships hermetic entry-signing tests at
...
- **T-SR-SIG-12:**This revision codec parity — a signed envelope emitted by
  `envelope_with_signature` is byte-equal to the corresponding
  unsigned this revision envelope EXCEPT for the one optional `signature`
  slot. Stripping the slot from the signed envelope and re-decoding
  through the codec recovers the original entry byte-equal
  (the codec is unchanged in v0.6.0).

Total v0.6.0 test additions: 12 hermetic determinism
...
### 6.6 kid-resolver tests (v0.7.0, additive over v0.6.0)

v0.7.0 ships hermetic resolver tests at
...
- **T-KID-RES-06** — wrong-alg filter. A `secp256k1` entry on the
  ...
  Reinforces Z-1-K-3: secp256k1 entries are invisible to
...
Total v0.7.0 test additions: 12 hermetic determinism
...
**Suite-level effect (post-v0.7.0):** the wirelang test
suite grows from 671 passed (post-v0.6.0) to **683 passed**
(+12 net). The entry-signing tests (T-SR-SIG-01..12) remain
unchanged and green.

### 6.7 AIP-document transport-fetch tests (v0.8.0, additive over v0.7.0)

v0.8.0 ships hermetic transport-fetch tests at
...
- **T-AIP-FT-11** — cross-layer composition. End-to-end:
  ...
  Pairs with v0.7.0 `T-KID-RES-03` (the resolver →
...
Total v0.8.0 test additions: 12 hermetic determinism
...
RFC 8032 Ed25519 test-vector seeds are reused from the v0.7.0
kid-resolver tests for the cross-layer composition probe.

**Suite-level effect (post-v0.8.0):** the wirelang test
suite grows from **683 passed** (post-v0.7.0) to **695 passed**
(+12 net). The entry-signing tests (T-SR-SIG-01..12) and the
This revision kid-resolver tests (T-KID-RES-01..12) remain unchanged and
green.

### 6.8 AIP-document signature-verification cache tier tests (v0.9.0, additive over v0.8.0)

v0.9.0 ships hermetic verification-cache tests at
...
test-vector seeds 1 and 2 are reused from the v0.7.0 / this revision
test fixtures.
...
Total v0.9.0 test additions: 12 hermetic determinism
...
are reused from the v0.7.0 / this revision fixtures.

**Suite-level effect (post-v0.9.0):** the wirelang test
suite grows from **695 passed** (post-v0.8.0) to **707 passed**
(+12 net). The entry-signing tests (T-SR-SIG-01..12), the
This revision kid-resolver tests (T-KID-RES-01..12) and this revision
transport-fetch tests (T-AIP-FT-01..12) remain unchanged and green.

### 6.9 `registered_by`-capability-gating tests (v0.10.0, additive over v0.9.0)

v0.10.0 ships hermetic capability-gating tests at
...
Total v0.10.0 test additions: 12 hermetic test
...
**Suite-level effect (post-v0.10.0):** the wirelang test
suite grows from **707 passed** (post-v0.9.0) to **752 passed**
(+45 net). The entry-signing tests (T-SR-SIG-01..12), the
This revision kid-resolver tests (T-KID-RES-01..12), this revision
transport-fetch tests (T-AIP-FT-01..12), and this revision
verification-cache tests (T-AIP-SVC-01..12) remain unchanged and
green.

### 6.10 Publisher-CLI capability integration tests (v0.11.0, additive over v0.10.0)

v0.11.0 ships hermetic tests at
...
- **T-SR-PUB-CG-10:** receipt-shape forward-compat — bare publish
  ...
  `gate_decision=null`; all pre-existing fields remain present;
...
**Suite-level effect (post-v0.11.0):** the wirelang test
suite grows from **752 passed** (post-v0.10.0) to
**780 passed, 1 skipped, 7 subtests passed** (+28 net through the
T-SR-PUB-CG-01..10 family and auxiliary coverage). This revision
This revision publisher-CLI inventory (T-SR-PUB-01..12) and this revision
This revision capability-gating inventory (T-RBC-01..12) remain unchanged
and green; the v0.11.0 tests are additive and exercise a
parallel test module.

#### Bucket policy source tests (v0.13.0, additive)

v0.13.0 ships hermetic tests at
...
**Suite-level effect (post-v0.13.0):** the wirelang test
suite grows from **804 passed, 1 skipped, 7 subtests passed**
(post-v0.12.0) to **818 passed, 1 skipped, 7 subtests passed**
(+14 net through the T-SR-PUB-CB-01..10 family and the auxiliary
class). The v0.4.0 publisher-CLI inventory
(T-SR-PUB-01..12), the v0.10.0 capability-gating inventory
(T-RBC-01..12), the v0.11.0 inventory (T-SR-PUB-CG-01..10),
and the v0.12.0 inventory (T-CPP-01..10) remain unchanged and
green; the v0.13.0 tests are additive and exercise a parallel
test module.

### 6.11 Capability-policy persistent distribution tests (v0.12.0, additive over v0.11.0)

v0.12.0 ships hermetic tests at
`wirelang/tests/test_capability_policy_nats_kv_backend.py`. The
inventory mirrors the v0.1.0
...
- **T-CPP-09:** a multi-policy-per-issuer bucket (two policies for
  ...
  registry where the v0.10.0 gate
...
- **`TestAuxEnvelopeShape`** (auxiliary probes): missing required
  ...
  empty `allowed_kids` rejected via the v0.10.0
...
**Suite-level effect (post-v0.12.0):** the wirelang test
suite grows from **780 passed** (post-v0.11.0) to
**804 passed, 1 skipped, 7 subtests passed** (+24 net through the
T-CPP-01..10 family and the two auxiliary classes). This revision
This revision schema-registry-backend inventory (T-SR-01..10 + 2 aux), the
v0.10.0 capability-gating inventory (T-RBC-01..12), and the
v0.11.0 publisher-CLI-capability-integration inventory
(T-SR-PUB-CG-01..10 + aux) all remain unchanged and green; the
v0.12.0 tests are additive and exercise a parallel test
module.

### 6.12 Capability-policy CAS-pin tests (v0.14.0, additive over v0.13.0)

v0.14.0 ships hermetic tests at
`wirelang/tests/test_capability_policy_cas_pin.py`. The inventory
is a **pattern-mirror** on the v0.2.0
...
- **T-CPP-CAS-06:** gate-2 (pair ↔ key derivation) runs BEFORE the
  CAS call. The v0.12.0 `CapabilityPolicyRecord`
...
**Suite-level effect (post-v0.14.0):** the wirelang test
suite grows from **818 passed, 1 skipped, 7 subtests passed**
(post-v0.13.0) to **830 passed, 1 skipped, 7 subtests
passed** (+12 net through the T-CPP-CAS-01..10 family and the two
auxiliary probes). The v0.2.0 schema-registry CAS-pin
inventory (T-SR-CAS-01..10 + 2 aux), the v0.12.0
capability-policy LWW inventory (T-CPP-01..10 + 2 aux), and the
v0.13.0 publisher-CLI bucket-source inventory
(T-SR-PUB-CB-01..10 + aux) all remain unchanged and green; the
v0.14.0 tests are additive and exercise a parallel test
module.

### 6.13 Capability-policy watch-stream tests (v0.15.0, additive over v0.14.0)

v0.15.0 ships hermetic tests at
...
on the v0.3.0 schema-registry watch-stream test inventory at
...
- **T-CPP-WS-09:** a frozen registry from a watch-fed snapshot gates
  identically to the full-snapshot path against the v0.10.0
...
v0.15.0; v0.15.0 brings the count to **842** (+12
net). The v0.3.0 schema-registry watch-stream inventory
(T-SR-WS-01..10 + 2 aux), the v0.12.0 capability-policy LWW
inventory (T-CPP-01..10 + 2 aux), the v0.13.0 publisher-CLI
bucket-source inventory (T-SR-PUB-CB-01..10 + aux), and the
v0.14.0 capability-policy CAS-pin inventory
(T-CPP-CAS-01..10 + 2 aux) all remain unchanged and green; the
v0.15.0 tests are additive and exercise a parallel test
module.

### 6.14 Capability-policy revocation tests (v0.16.0, additive over v0.15.0)

v0.16.0 ships hermetic tests at
...
- **T-CPP-REV-05:** the gate denies with `POLICY_REVOKED` even when
  ...
  (v0.10.0 window semantics): the validity-window check
...
- **T-CPP-REV-08:** an older this revision envelope (omitting
  the two new keys entirely) decodes via the v0.16.0 decoder
...
- **T-CPP-REV-aux-lww-allows-unrevoke:** the v0.12.0 LWW
  ...
  CAS-pin path, consistent with the v0.14.0 rationale.

**Suite-level effect.** `wirelang/tests/` was 842 passing before
v0.16.0; v0.16.0 brings the count to **854** (+12
net). The v0.10.0 `registered_by`-capability-gating
inventory (T-RBC-01..12), the v0.12.0 capability-policy LWW
inventory (T-CPP-01..10 + 2 aux), the v0.13.0 publisher-CLI
bucket-source inventory (T-SR-PUB-CB-01..10 + aux), this revision
This revision CAS-pin inventory (T-CPP-CAS-01..10 + 2 aux) and the
v0.15.0 watch-stream inventory (T-CPP-WS-01..10 + 2 aux) all
remain unchanged and green; the v0.16.0 tests are additive
...
- V-908 conflict-error pattern source:
  `RouteRegistryConflictError` in the same module (this revision mirror).
- V-908 watch-stream pattern source: This revision `WatchOp` / `WatchEvent` /
  `LiveSnapshot.from_backend` / `_MockWatcher` in the same module
  (this revision mirror).
- Bucket inventory source:
  `scripts/init-nats-buckets.py` `PHASE_1_BUCKETS[0]` (`wakir-schemas`).
- **Phase-1c CAS-pin: OI-7-Phase-1c-CAS — CONSUMED in this revision.**
- **Phase-1c watch-stream: OI-7-Phase-1c-watch — CONSUMED in this revision.**
- **Phase-1c publisher CLI: OI-7-Phase-1c-publisher — CONSUMED in
This revision.** Module: `wirelang/schemas/publisher_cli.py`. Tests:
  `wirelang/tests/test_schema_registry_publisher_cli.py`.
- **Phase-1c cross-bucket replication: OI-7-Phase-1c-replication
  — CONSUMED in this revision.** Module: `wirelang/schemas/replication.py`.
  Tests: `wirelang/tests/test_schema_registry_replication.py`.
- **Phase-1c is feature-complete; all four Phase-1c slots are
  consumed.**
- **Phase-2 entry-signing: OI-7-Phase-2-sig — CONSUMED in this revision
This revision.** Module: `wirelang/schemas/entry_signing.py`. Tests:
  ...
  touch).
- **Phase-2 kid → public-key resolver: CONSUMED in this revision.**
  Module: `wirelang/identity/kid_resolver.py`. Tests:
  `wirelang/tests/test_identity_kid_resolver.py` (T-KID-RES-01..12).
  Closes Z-1-K-1 (kid-Resolver-Shape) and reinforces
  Z-1-K-3 (Curve-Choice = Ed25519). The resolver is the
  Identity-Substrate consumer of v0.6.0 signature blocks:
...
- **Phase-2 AIP-document transport-fetch: CONSUMED in this revision
This revision.** Module: `wirelang/identity/aip_document_transport_fetch.py`.
  Tests: `wirelang/tests/test_aip_document_transport_fetch.py`
  (T-AIP-FT-01..12). Closes the v0.7.0 §5.9 boundary item
  "AIP-document transport-fetch (`did:web` / `aip:web` HTTPS bridge)".
...
- **Phase-2 AIP-document signature-verification cache tier: CONSUMED
  ...
  Substrate slot in §5.3; v0.9.0 ships it now to round out
  ...
  v0.6.0 `wirelang.identity.verify_aip_signature`
  ...
  v0.8.0 `jcs_sha256_hex` byte-anchor (Z-1-K-2
...
- **Phase-2 `registered_by`-capability-gating: CONSUMED in this revision
This revision.** Module: `wirelang/schemas/registered_by_capability.py`.
  Tests: `wirelang/tests/test_registered_by_capability.py`
  (T-RBC-01..12; 45 sub-tests). Previously listed as a follow-up
  Phase-2 slot in §5.8 boundary block (v0.6.0 noted that
  capability-token gating on `registered_by` was reserved); this revision
  ...
  top of the v0.6.0 `verify_entry_signature` cryptographic
...
- Phase-2 Biscuit binary token encode/decode + Datalog evaluation:
  reserved (Phase-3 slot; the v0.10.0 gate is the in-process
...
- ~~Phase-2 capability-policy distribution (persisted NATS-KV
  ...
  (Phase-3 slot; the v0.10.0 registry is in-process only).~~
  (**CONSUMED in v0.12.0** by
  `wirelang.schemas.capability_policy_nats_kv_backend`; see §5.14.
  The v0.12.0 module ships the persistent-distribution tier
  for capability policies on the `wakir-capability-policies` bucket;
  the v0.10.0 in-process `CapabilityPolicyRegistry` remains
  ...
  upserts (LWW-only in v0.12.0; Phase-3 slot) and watch-stream
  tail (full-snapshot only in v0.12.0; Phase-3 slot) remain
  follow-up slots. ~~Publisher-CLI `--capability-bucket` integration
  (this revision candidate)~~ **CONSUMED in v0.13.0** via
  ...
  see §5.13's "Bucket policy source (v0.13.0)" subsection and
  §6.10's "Bucket policy source tests" addendum.)
- ~~Phase-2 publisher-CLI integration of the v0.10.0 gate:
  reserved (`wakir-schema-registry publish` flag that runs
  `gate_signed_entry` between `sign_entry` and `put`).~~
  **CONSUMED in v0.11.0** (`wirelang/schemas/publisher_cli.py`
  ...
  contract; §6.10 ships T-SR-PUB-CG-01..10). v0.11.0
  boundary leaves the on-the-wire envelope UNCHANGED (no signature
  on the bucket; that is a slot).
- **Phase-2 capability-policy CAS-pin: CONSUMED in this revision.**
  Module: `wirelang/schemas/capability_policy_nats_kv_backend.py`
  (additive on the v0.12.0 module; the LWW path is byte-
  unchanged). Tests:
  `wirelang/tests/test_capability_policy_cas_pin.py`
  (T-CPP-CAS-01..10 + 2 aux). Closes the v0.12.0 §5.14
  ...
  The v0.12.0 LWW `put` path is byte-unchanged and remains
  ...
  slot~~ (**CONSUMED in v0.15.0** — see §5.14 Watch-stream
  ...
  reserved) remain on the Phase-2/Phase-3 boundary; this revision
This revision watch-stream is the live-tail consumer substrate, not a
  CAS-quorum promotion. This revision candidate: publisher-CLI
  composition of the live-tail consumer (a `--capability-bucket-watch`
  flag or daemon-mode subcommand) — explicitly NOT this revision.
- Phase-2 STRICT-mode activation toggle: reserved (Z-1-K-4
  open; operator-controlled toggle is a Phase-2-roadmap consensus
  question).
- **Phase-2 capability-policy explicit revocation: CONSUMED in
This revision.** Modules:
  ...
  Phase-3 slot. This revision candidates: publisher-CLI `--revoke`
  flag composing the CAS-pin revocation path (CONSUMED in this revision
This revision, v0.17.0); watch-stream consumer-side revocation-event
  filter helper (CONSUMED in v0.18.0, v0.18.0); cross-bucket
  revocation replication (CONSUMED in v0.19.0, v0.19.0 — see
...
This revision: no on-disk loader is removed, no verifier module
...
deployment will switch to; v0.1.0 lands the
substrate, This revision hardens it with the CAS-pin path. The cutover
itself is still Phase-2.

**v0.2.0 is additive relative to v0.1.0:**

- All This revision surfaces (`get` / `put` / `delete` / `list_keys` /
...
- All This revision surfaces remain unchanged. This revision introduces
...
- All This revision surfaces remain unchanged. this revision
...
- All This revision surfaces remain unchanged.
...
This revision LWW + this revision CAS-pin + this revision watch-stream surfaces; it
...
- All This revision surfaces remain
  unchanged. v0.6.0 introduces no new method on
  `NatsKvSchemaRegistry`.
- The v0.6.0 addition is a *separate module*
  ...
  `VerifyMode`. The signing module is a *consumer* of this revision
...
- The envelope codec (`_entry_to_envelope` /
  `_envelope_to_entry`) is left UNCHANGED in this revision. The
...
- M-2 conformance (additive-only schema evolution): v0.6.0
...
- M-4 conformance (multi-version-aware registry): v0.6.0
...
- All this revision + v0.6.0 surfaces
  remain unchanged. v0.7.0 introduces no new method on
  `NatsKvSchemaRegistry` and no modification to
  `wirelang.schemas.entry_signing`.
- The v0.7.0 addition is a *separate module*
...
- The `wirelang.schemas.entry_signing` module is UNCHANGED. The
  ...
  module does NOT import the resolver; this preserves this revision
This revision design that the signing module treats the public key as
  caller-supplied.
- M-2 conformance (additive-only schema evolution): v0.7.0
...
- M-4 conformance (multi-version-aware registry): v0.7.0
...
- All this revision + v0.6.0 +
  v0.7.0 surfaces remain unchanged. v0.8.0 introduces
  no new method on `NatsKvSchemaRegistry`, no modification to
  `wirelang.schemas.entry_signing`, and no modification to
  `wirelang.identity.kid_resolver` (the v0.7.0 surface).
- The v0.8.0 addition is a *separate module*
...
- M-2 conformance (additive-only schema evolution): v0.8.0
...
- M-4 conformance (multi-version-aware registry): v0.8.0
...
- Cross-Review-Zone-1 (Identity-Substrate) non-touched: the four
  Z-1-K consensus points remain byte-identical. This revision
  ...
  from the v0.6.0 `VerifyMode.STRICT` signature-policy toggle
  (Z-1-K-4).
- The synchronous verifier surface (`InMemorySchemaRegistry.lookup`
...
- All this revision + v0.6.0 +
  ...
  correctness).
- The v0.9.0 addition is a *separate module*
...
- M-2 conformance (additive-only schema evolution): v0.9.0
  adds NO new envelope field. The schema-registry on-the-wire
  surface (envelope shape, value-schema URI
  `wakir.wirelang.schema-registry-entry/1`, signature-block shape)
  is bit-equal to v0.8.0. M-2 is preserved trivially.
- M-4 conformance (multi-version-aware registry): v0.9.0
...
- Cross-Review-Zone-1 (Identity-Substrate) non-touched: the four
  Z-1-K consensus points remain byte-identical. The cache
  is curve-agnostic (Z-1-K-3 non-touched / reinforced),
  policy-agnostic (Z-1-K-4 non-touched), and
  resolver-independent (Z-1-K-1 non-touched — the cache
  takes a `pub_key` argument the caller has already resolved).
  Z-1-K-2 (JCS-Resolver-Lock) is preserved byte-identical
...
- All v0.6.0 + This revision surfaces remain
...
This revision entry-signing layer directly; they are not forced onto
  the gate.
- M-2 conformance (additive-only schema evolution): This revision adds no
  ...
  separate at this revision: the in-process bundle ships now; the
...
- All v0.1.0 / This revision surfaces and
  all v0.6.0 / This revision surfaces remain
  unchanged. Their contracts are preserved byte-equal.
- The v0.11.0 additions are *operator-CLI* surface extensions
  ...
  `_parse_optional_rfc3339`, `_decision_to_dict`). Pre-this revision
  invocations (no capability flags) emit a receipt with
  `signed=false`, `kid=null`, `gate_decision=null` and exit codes
  in the byte-equal pre-existing matrix; no caller is forced onto
  the capability path.
- M-2 conformance (additive-only schema evolution): v0.11.0
  ...
  is REFERENCED only; the v0.11.0 file format is a separate,
  simpler operator-side surface intended for Phase-3 promotion to
  the Biscuit envelope.
- M-4 conformance (multi-version-aware registry): v0.11.0 is
...
- Receipt-shape additivity: the three new optional receipt fields
  ...
  pre-existing field set continue to read byte-equal pre-existing
  ...
  byte-for-byte across v0.10.0 and v0.11.0 receipts
  WILL see a difference (the three new keys), but consumers that
  filter to the pre-existing key set will not. The minor-version
  bump reflects this.
- Cross-Review-Zone-1 (Identity-Substrate) **non-touched**: the four
  Z-1-K consensus points remain byte-identical after
This revision. The integration is curve-agnostic at the
  CLI-flag layer (operator supplies the seed; the underlying
  `sign_entry` call is byte-identical to this revision); JCS-free at the
...
- `entry_signing.sign_entry`, `entry_signing.verify_entry_signature`,
  ...
  the pre-existing unsigned path. The signature block is consumed
  CLI-side only.
...
- A new module `wirelang.schemas.capability_policy_nats_kv_backend`
  is introduced. The v0.11.0 module
  `wirelang.schemas.publisher_cli` is byte-unchanged; this revision
This revision module `wirelang.schemas.registered_by_capability` is
  byte-unchanged; the v0.6.0 module
  `wirelang.schemas.entry_signing` is byte-unchanged; this revision
This revision modules
...
- A new dedicated NATS-KV bucket `wakir-capability-policies` is
  ...
  byte-unchanged. The new bucket is the 7th slot on the DevOps track's
  `PHASE_1_BUCKETS` inventory pending the Z-B paired-update
  (this revision DevOps-side slot).
...
- Cross-Review-Zone-1 non-touched: the four Z-1-K
...
- Cross-Review-Zone-B **TRIGGERED**: the orchestrator-side
  ...
  side `BucketSpec` will mirror it byte-precisely on DevOps-side
  ...
  the live cluster.
- The v0.10.0 in-process `CapabilityPolicyRegistry` remains
  the canonical evaluation surface. v0.12.0 ships the
  source-of-truth substrate that materialises it via
  `NatsKvCapabilityPolicyBackend.snapshot_registry`. This revision
This revision `check_registered_by_capability` function is byte-unchanged
  ...
  JSON (v0.11.0 path) or materialised from the persistent
  bucket (v0.12.0 path).
- The v0.11.0 publisher-CLI `--capability-registry` flag is
  byte-unchanged: it still reads operator-local JSON. A future
  `--capability-bucket` flag that reads policies from the
  v0.12.0 bucket is a candidate; this revision
  This revision is the substrate, not the CLI integration.
- The v0.12.0 backend ships PUT (LWW) only. A CAS-pin tier
  (`put_with_revision`) is reserved as a Phase-3 slot mirroring
  the v0.2.0 schema-registry CAS-pin pattern.
- The v0.12.0 backend ships full-snapshot only. A
  watch-stream tail (`watch()` / `WatchEvent` /
  `LiveCapabilityPolicySnapshot`) is reserved as a Phase-3 slot
  mirroring the v0.3.0 schema-registry watch-stream
  pattern.
...
- The `PublishReceipt` dataclass gains a fourth optional field
  `gate_policy_source: Optional[str]` (default `None`). Pre-this revision
...
- The receipt's `gate_policy_source` is `"file"` for this revision
This revision file path, `"bucket"` for the new v0.13.0 bucket
...
- The v0.12.0 `NatsKvCapabilityPolicyBackend` surface is
  byte-unchanged. v0.13.0 consumes
  `snapshot_registry()` exactly once per CLI invocation; no `put`,
  `delete`, or `watch` is invoked from the CLI path.
- The v0.12.0 bucket `wakir-capability-policies` is consumed
  as-is. No new bucket is requested; Cross-Review-Zone-B is
  non-touched at the v0.13.0 axis (the v0.12.0
...
- The schema-registry bucket (`wakir-schemas`) write path is
  ...
  the v0.11.0 file-source short-circuit (verified by
  T-SR-PUB-CB-03..05).
...
- All v0.12.0 LWW surfaces (`put` / `get` / `get_by_pair` /
...
- Cross-Review-Zone-B (the DevOps-track bucket inventory) non-touched: no new
  bucket; the `wakir-capability-policies` bucket configuration is
  byte-unchanged; the v0.12.0 paired-update memo remains
  the canonical Z-B trigger.
- The v0.13.0 publisher-CLI `--capability-bucket` flag is
  read-only (`snapshot_registry()`) so the CAS-pin write path is
  not reachable from the operator CLI in this revision. A
  `--capability-bucket-cas-rotate` (or similar) subcommand surface
  is a candidate, explicitly NOT shipped in a separate revision.
- The v0.2.0 schema-registry CAS-pin surface is
  byte-unchanged. The v0.14.0 surface is a parallel module-
...
- The Phase-3 capability-policy watch-stream slot (full-snapshot
  only in v0.12.0/3/4; live-tail is Phase-3) and the
  Phase-3 multi-replica CAS-quorum slot (single-replica
  `replicas=1` in v0.12.0/3/4 configuration) remain
  reserved.
- Spec semver bump 0.13.0 → 0.14.0 reflects the additive minor
  ...
  new §5.14 "CAS-pin operational contract (v0.14.0)"
...
- The v0.15.0 additions to
  ...
  - New enum `CapabilityPolicyWatchOp` with values `PUT`, `DELETE`,
    ...
    `WatchOp` (v0.3.0); the class identity is module-local
  ...
  - New live-tail consumer dataclass
    ...
    frozen v0.10.0 `CapabilityPolicyRegistry` view in sorted-
  ...
  - Module docstring boundary-item "live tail reserved as a
    Phase-3 slot" rewritten to "added in v0.15.0
    (pattern-mirror on the schema-registry watch-stream, this revision
This revision)" with the orthogonality stamp (full-snapshot path
    remains supported and orthogonal).
- All v0.12.0 LWW surfaces (`put` / `get` / `get_by_pair` /
  `delete` / `list_keys` / `snapshot` / `snapshot_registry`) and all
  v0.14.0 CAS-pin surfaces (`get_with_revision` /
  ...
  (v0.12.0) and T-CPP-CAS-01..10 + 2 aux (v0.14.0)
...
- Cross-Review-Zone-B (the DevOps-track bucket inventory) non-touched: no new
  bucket; the `wakir-capability-policies` bucket configuration is
  byte-unchanged; the v0.12.0 paired-update memo remains the
  canonical Z-B trigger.
- The v0.13.0 publisher-CLI `--capability-bucket` flag is
  read-only (`snapshot_registry()`) and the v0.15.0
  watch-stream is NOT reachable from the operator CLI in this revision.
...
- The v0.3.0 schema-registry watch-stream surface
  (`WatchOp`, `WatchEvent`, `NatsKvSchemaRegistry.watch`,
  `LiveSchemaSnapshot`, `open_watch_stream`) is byte-unchanged. The
  v0.15.0 surface is a parallel module-local addition; the
  schema-registry helpers are not re-exported, re-shimmed, or
  refactored. The v0.15.0 module-local copies preserve the
  evolution-decoupling stance set in v0.14.0 for the CAS-pin
  helpers.
- Phase-3 reservations preserved:
  - Watch-stream resumption / replay-from-revision
    (`watchall(..., resume_from=...)`) — nats-py supports it; the
    v0.15.0 wrapper exposes the underlying revision via
  ...
  - Cross-bucket capability-policy replication (analogous to
    v0.5.0 schema-registry replication) — This revision lands the
    watch-stream substrate; the replication-tier composition is
    reserved.
  - Multi-replica CAS-quorum — `replicas=1` v0.12.0 bucket
...
- Spec semver bump 0.14.0 → 0.15.0 reflects the additive minor
  ...
  new §5.14 "Watch-stream operational contract (v0.15.0,
  additive over v0.14.0)" subsection, and the new §6.13
...
content). The v0.6.0 additions (§5.8, §6.5, change-log
v0.6.0 entry, Phase-2 boundary updates) use role-strings only
(`the Wirelang track`, `the WAT track`, `the operator`, `HR` appear in outbox documents and
optional cross-review memos, not in this spec; the spec mentions
only `wakir.*` URIs and module-path references).

The v0.7.0 additions (§5.9, §6.6, change-log v0.7.0 entry,
§1.2 v0.7.0 block, §7 cross-references update, §8
...
The v0.8.0 additions (§5.10, §6.7, change-log v0.8.0 entry,
§1.2 v0.8.0 block, §5.3 lands-update,
...
The v0.9.0 additions (§5.11, §6.8, change-log v0.9.0 entry,
§1.2 v0.9.0 block, §5.3 lands-update — this revision
An earlier increment added to the §5.3 header, AIP-document signature-verification
cache tier item moved from Phase-3-reserved to CONSUMED-in-this revision-
This revision, §7 cross-references update — verification-cache tier slot
...
no internal-persona-clear-name leakage in the spec body
additions.

The v0.10.0 additions (§5.12, §6.9, change-log v0.10.0 entry,
§1.2 v0.10.0 block, §5.3 lands-update — this revision
An earlier increment added to the §5.3 header, `registered_by`-capability-gating
item moved from follow-up-Phase-2-slot (§5.8 v0.6.0
boundary) to CONSUMED-in-this revision, §7 cross-references update
...
leakage in the spec body additions.

The v0.11.0 additions (§5.13, §6.10, change-log v0.11.0 entry,
§5.3 publisher-CLI-capability-integration-slot CONSUMED, §5.6
publisher-CLI capability-flag cross-reference, §5.12 boundary
publisher-CLI item CONSUMED, §7 publisher-CLI-integration-of-gate
...
The v0.12.0 additions (§5.14, §6.11, change-log v0.12.0
...
The v0.13.0 additions (§5.13 "Bucket policy source (this revision
This revision)" subsection, §6.10 "Bucket policy source tests" addendum,
...
(`wakir-capability-policies` carried unchanged from v0.12.0;
...
internal-persona-clear-name leakage in the spec
body additions.

The v0.14.0 additions (§5.14 "CAS-pin operational contract
(v0.14.0)" subsection, §6.12 capability-policy CAS-pin test
...
The v0.15.0 additions (§5.14 "Watch-stream operational
contract (v0.15.0, additive over v0.14.0)" subsection,
§5.14 "Boundary: v0.15.0 does NOT ship (explicit)" subsection,
...
The v0.32.0 additions (§10 "Persona-Engine-Format
...
internal-persona-clear-name leakage in the v0.32.0
spec body additions.

## 10. Persona-Engine-Format integration (v0.32.0)

### 10.1 Scope

This section anchors the v0.32.0 Persona-Engine-Format
...
sprint; This revision keeps them as filesystem-only artefacts to keep the
...
This is the **load-bearing decision** of v0.32.0:
...
| Zone | Partner | this revision touch | this revision ack required |
|---|---|---|---|
| J (container-bridge) | container-ops-slot | shape-only (image template, labels, env vars in §3.5 of persona-engine-format-spec) | yes, before |
...
| L (identity-substrate) | identity-eng-slot | reservation-only (synthesised hierarchy default in §4.3) | not until |
| B (NATS-KV × Wirelang) | container-ops-slot (bucket inventory) | reservation-only (`wakir-persona-state-{persona_id}` family documented; no bucket configured) | not until |
| HR (governance) | hr-slot | active (synthesis defaults HR-slot-ratification-pending) | yes, before |

### 10.6 Compatibility statement (v0.32.0 → v0.32.0)

v0.32.0 is purely additive over v0.31.0:

- All this revision schema-registry surfaces (`get` / `put` /
...
## 11. Operator-CLI annex (v0.35.0 Part A)

### 11.1 Scope

This annex anchors the Teil A operator-facing
marker-stack reducer CLI into the schema-registry spec at
v0.35.0. The CLI wraps the v0.30.0 in-memory reducer
(`wirelang.federation.marker_composition.reduce_marker_stack`)
plus the v0.31.0 durable backend
...
### 11.8 Compatibility statement (v0.34.0 → v0.35.0)

v0.35.0 is purely additive over v0.34.0:

- All v0.30.0 / This revision surfaces (`reduce_marker_stack`,
...
- All v0.33.0 / This revision surfaces (`detect_replay`,
...
- The v0.28.0 `MultiOrgAttestationReplicator`
