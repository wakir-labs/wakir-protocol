...
- **Inverse migration `v_{n+1} -> v_n`.** Migration is intentionally
  not invertible (the design sketch §2.2 Regel B2 / §4.4 marker M-3). A
...
|---|---|---|
| **A-1** | The mock canonical subset is the input shape (this revision baseline). | Steps consume and emit canonical-subset dicts; raw markdown is parsed once at the public-API boundary by `_coerce_to_dict`. |
...
The converter is byte-deterministic by construction. the design sketch §2.3
...
The active registry as of This revision carries two entries,
`V0ToV1Step` and `V1ToV2Step`, each performing only the
schema-version lift (the design sketch §3.2 R1-R4 and this revision
...
The implementation ships with five converter
test files:

| File | Coverage | Provenance |
|---|---|---|
| `tests/test_persona_migration.py` | Public-API happy paths (S2-T1-01, S2-T1-02). | this revision2 |
| `tests/test_persona_migration_edge_cases.py` | Defensive belt: degraded front-matter, malformed inputs, immutability, cycle-guard (S2-T1-03). | this revision |
| `tests/test_persona_migration_roundtrip.py` | Cross-version round-trip + idempotence + input-shape-equivalence (S2-T1-04). | this revision |
| `tests/test_persona_migration_cli.py` | Operator CLI (`wakir-persona migrate`) v1-target surface. | this revision |
| `tests/test_persona_migration_v1_to_v2.py` | `V1ToV2Step` direct + V0->V2 chain pin-match + M-1 direct anchor + non-invertibility negative. | this revision |
| `tests/test_persona_migration_cli_v2_target.py` | Operator CLI `--target persona-v2`: single-step v9->v2, multi-step v8->v0->v1->v2 chain, `--expect-hash` against the V8/V9-MIGRATED-V2 pin, forward-only-rejection negative. | this revision |

The existing fixtures `wirelang/tests/fixtures/persona_definitions/v8-persona-pre-framework.md`
and `v9-persona-framework-native.md` form the round-trip
anchor (`v8 -> v1` reproduces `PERSONA_HASH_PIN_V9`) and this revision
...
## 9. Cross-review zones (status)

| Zone | Partner | Status | Trigger |
|---|---|---|---|
| **K** (WAT-Hash V-907) | wat-eng-slot | tracking only | Phase-2 item P2-01 (migration-audit-trail in WAT-leaf-slot). No converter-side change required for this revision. Reserved-field slot `wat_bridge_overrides` carries zero-semantics in v2. |
...
The Phase-1c Rust re-write of `persona_hash.py` is scheduled (this revision
