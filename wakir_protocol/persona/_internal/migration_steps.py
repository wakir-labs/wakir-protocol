...
by composition in :mod:`wakir_protocol.persona.persona_migration`.
...
:mod:`wakir_protocol.persona.persona_migration` performs an exact-match
...
This module mock canonical subset, not a content extension. With the
This module ``V1ToV2Step`` registered below, v0 inputs resolve
...
This module (``wirelang/schemas/persona-v2.json``) opens a closed
    ...
    - **Reserved fields** (Option D-A pin-stability discipline,
      ...
      mappings. The earlier reserved-field round-trip pack
    ...
    *includes* ``schema_version`` in the JCS bytes, so lifting v1 to v2
    ...
    M-1 (linear-chain) direct-anchor that an earlier increment flagged as
    currently indirect.
    """
...
#:
#: An earlier increment introduced :class:`V0ToV1Step`.
#: This module appends :class:`V1ToV2Step` (linear-chain
