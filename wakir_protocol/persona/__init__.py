...
pending as of this module). The mock subset is engine-default;
...
(the Wirelang track Cross-Review Zone-B B-5). The lazy-import pattern is
parity with the ``wirelang.identity`` disentanglement
(this module Bug-6 fix, PEP-562 ``__getattr__``).
...
def __getattr__(name: str) -> Any:
    ...
    if name in _LAZY_CRYPTO_ATTRS:
        ...
        submodule = import_module(f"wakir_protocol.persona.{submodule_basename}")
...
def __dir__() -> list[str]:
    ...
    autocomplete parity with the pre-existing eager-import shape.
    """
...
if TYPE_CHECKING:  # pragma: no cover - type-checker only
    from wakir_protocol.persona.persona_hash import (  # noqa: F401
    ...
    from wakir_protocol.persona.persona_migration import (  # noqa: F401
        PERSONA_SCHEMA_VERSION_LATEST,
        PERSONA_SCHEMA_VERSION_LIST,
        PersonaMigrationDeterminismError,
    ...
    from wakir_protocol.persona.persona_validator import (  # noqa: F401
        PERSONA_VALIDATION_REPORT_SCHEMA_VERSION,
