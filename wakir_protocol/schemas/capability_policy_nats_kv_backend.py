...
:mod:`wakir_protocol.schemas.registered_by_capability`
...
- Synchronous in-memory snapshot view via
  ...
  materialises a
  :class:`wakir_protocol.schemas.registered_by_capability.CapabilityPolicyRegistry`
  directly. Verifier-side code (the
  :func:`check_registered_by_capability` gate) consumes the
  :class:`CapabilityPolicyRegistry` byte-identical to the
  in-process path; the only difference is the registry origin
...
1. The :class:`CapabilityPolicyRecord` round-trips through the
   :class:`wakir_protocol.schemas.registered_by_capability.CapabilityPolicy`
...
evaluation (gate decisions) remains the in-process this module
...
- This module ships the persistent-distribution tier for capability
  ...
  :class:`CapabilityPolicyRegistry` from this module: operators
  who prefer to keep policies in operator-local JSON files (the
This module ``--capability-registry`` flag) continue to use
...
- This module DOES NOT auto-distribute policies to publisher-side
  ...
  startup (or on a periodic refresh schedule). This module
  slot was full-snapshot only; the live tail
  (:meth:`NatsKvCapabilityPolicyBackend.watch` /
  :class:`LiveCapabilityPolicySnapshot`) is **added in a later revision**
  (pattern-mirror on the schema-registry watch-stream)
...
- This module ships PUT (LWW) only. The CAS-pinned upsert path
  ``put_with_revision`` is **added in a later revision** (pattern-mirror
  on the schema-registry CAS-pin) — see
  :meth:`NatsKvCapabilityPolicyBackend.put_with_revision` and
  :class:`CapabilityPolicyConflictError` below. This module
...
- This module DOES NOT replace this module
  ...
  slot (later-increment candidate) can add a
  ``--capability-bucket`` flag that reads policies from this
  bucket; that integration is NOT part of this module.

Cross-Review-Zone-B interaction
-------------------------------

This module IS a Z-B Trigger. The orchestrator-side
:data:`PHASE_1_BUCKETS` inventory currently carries 6 buckets
(subsequent bucket addition). This module adds the
7th bucket ``wakir-capability-policies``. The paired-update memo to
the DevOps track lists the byte-precise
``BUCKET_CONFIG`` mirror contract; the orchestrator-side init script
addition is operator-tracked as a DevOps-side
...
- In-process registry source: :mod:`wakir_protocol.schemas.registered_by_capability`
  :mod:`wakir_protocol.schemas.registered_by_capability`.
- Pattern source: :mod:`wakir_protocol.schemas.registry_nats_kv_backend`
  :mod:`wakir_protocol.schemas.registry_nats_kv_backend` (mirror).
- Bucket inventory source: ``scripts/init-nats-buckets.py``
  ``PHASE_1_BUCKETS`` (this module currently lists 6 slots;
This module paired-update memo requests a 7th).
...
- Capability-Token-Layer ownership: Wirelang track.
"""
...
from wakir_protocol.schemas.registered_by_capability import (
    CapabilityPolicy,
...
#: This module convention; this name will be registered in
#: the DevOps track's :data:`PHASE_1_BUCKETS` as the 7th bucket via this module
#: This module paired-update memo (DevOps-track this module slot).
...
This module CAS-pin contract (pattern-mirror on the
This module schema-registry CAS-pin path; see
    :class:`wakir_protocol.schemas.registry_nats_kv_backend.SchemaRegistryConflictError`).
...
This module substance: two operators editing the same
    ``(registered_by, policy_id)`` pair concurrently — for example to
    rotate ``allowed_kids`` after a key-rollover — would otherwise risk
    one overwriting the other's edit silently under this module
This module last-write-wins ``put`` contract. CAS-pin makes the conflict
...
This module: revocation is a one-way state transition.
...
This module. The
    ...
    *structurally indistinguishable* on the policy axis from an
    ...
    Backward compatibility: every envelope written before this module
This module omits the marker; the decoder treats absence as ``None``
    ...
    Verifier-side gating (this module revocation
...
class CapabilityPolicyRecord:
    ...
    The :class:`CapabilityPolicy` bundle from This module carries
    ...
    - ``unrevoke_audit_marker``: This module additive
...
# This module: additive operator-deliberate-unrevoke
...
# This module invariant: the marker, if present,
...
def _record_to_envelope(record: CapabilityPolicyRecord) -> bytes:
    ...
    payload = {
        ...
        "note": policy.note,
# This module: additive revocation surface.
        # Both fields default to ``None`` (no revocation); this module
# This module envelope without these keys decodes byte-equally
...
# This module: optional revocation fields (additive).
    # Older this module envelopes omit these keys; the decoder
...
# This module: optional unrevoke-audit-marker (additive,
    ...
    # envelope error (not this module RegisteredByCapabilityError);
...
This module CAS-pin helper. The revision is the
        ...
        Mirror of
        :meth:`wakir_protocol.schemas.registry_nats_kv_backend.NatsKvSchemaRegistry.get_with_revision`.
        """
        if not isinstance(key, str) or not key:
            return None
        try:
...
This module CAS-pin (pattern-mirror on the
This module schema-registry CAS-pin path).
        ...
        try:
            ...
            # CAS-pin path forwards that failure up. Wirelang-track-Hand: a
    ...
    async def snapshot_registry(self) -> CapabilityPolicyRegistry:
        ...
        :class:`CapabilityPolicyRegistry` ready for this module
This module :func:`check_registered_by_capability` gate.
        ...
        :class:`InMemorySchemaRegistry.keys_sorted` this module
        determinism contract).

        Disabled policies ARE included in the registry; the gate
        evaluates them (this module semantics: a disabled policy
...
This module boundary: the watch-stream is a *consumer*
...
This module :func:`check_registered_by_capability` gate when
...
This module CAS-pin helper (pattern-mirror on the
...
# ---------------------------------------------------------------------------
# Watch-Stream-Snapshot Layer (this module)
# ---------------------------------------------------------------------------
#
# This module (S5-2) shipped get/put/delete/snapshot/snapshot_registry. The
...
# re-listing.
#
# This module (S5-5) adds a watch-based incremental layer that consumes
...
#   This module does not silently swallow envelope poison
...
class CapabilityPolicyWatchOp(enum.Enum):
    ...
    :class:`wakir_protocol.schemas.registry_nats_kv_backend.WatchOp`
...
class CapabilityPolicyWatchEvent:
    ...
    Pattern-mirror on
    :class:`wakir_protocol.schemas.registry_nats_kv_backend.WatchEvent`
...
def _decode_capability_policy_watch_update(
    ...
    :func:`wakir_protocol.schemas.registry_nats_kv_backend._decode_watch_update`
...
class _CapabilityPolicyWatchStreamHandle:
    ...
    Byte-equal source mirror of
    :class:`wakir_protocol.schemas.registry_nats_kv_backend._SchemaWatchStreamHandle`
...
async def _open_capability_policy_watcher(kv: Any) -> Any:
    ...
    Byte-equal source mirror of
    :func:`wakir_protocol.schemas.registry_nats_kv_backend._open_watcher`
...
async def open_capability_policy_watch_stream(
    ...
    Pattern-mirror on
    :func:`wakir_protocol.schemas.registry_nats_kv_backend.open_watch_stream`.
    """
...
This module (S5-5) substrate. Initialises an in-memory
    ...
    :class:`CapabilityPolicyRegistry` from This module is indexed by
    ...
    Pattern-mirror on
    :class:`wakir_protocol.schemas.registry_nats_kv_backend.LiveSchemaSnapshot`.
    """
...
# symmetry to backend revocation-axis)
# ---------------------------------------------------------------------------
#
# An earlier increment added the backend-write side of the revocation surface
# (the ``CapabilityPolicy.revoked_at`` + ``revocation_reason`` fields,
# the ``CapabilityPolicyRevocationConflict`` invariants on the write
# path, the publisher-CLI ``revoke`` subcommand from this module). The
...
#
# Symmetry with this module: the classifier mirrors the same
...
class RevocationEventKind(enum.Enum):
    ...
    Six kinds (An earlier increment introduced five, An earlier increment added
    ...
    - :attr:`REVOCATION_MONOTONIC_BREACH`: a PUT against a prior-revoked
      ...
      (advance or retreat). CAS-pin backend invariants from this module
This module forbid these transitions even via the LWW path's this module
This module unrevoke subcommand (which writes a fully-formed
...
class RevocationEventClassifier:
    ...
    def classify(
        ...
        Decision table (An earlier increment extended with marker column):
        ...
        if prior_revoked_at is None:
            ...
            if incoming_revoked_at is None:
                # Apparent un-revoke. This module: branch on the
...
This module operator-deliberate-unrevoke kind). Pass
    ...
    if kinds is None:
        kinds = frozenset({
            ...
            RevocationEventKind.REVOCATION_MONOTONIC_BREACH,
# This module: surface operator-deliberate unrevoke
