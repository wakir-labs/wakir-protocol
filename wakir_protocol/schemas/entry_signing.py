...
(:mod:`wakir_protocol.schemas.registry_nats_kv_backend`) with the
:mod:`wakir_protocol.identity_substrate.aip_signing` JCS + SHA-256 + Ed25519
...
2. **Canonicalise** the remaining payload with RFC 8785 JCS (resolver
   ...
   in :mod:`wakir_protocol.identity_substrate._jcs_pure` otherwise).
...
This module boundary (spec §5.8 Phase-2 boundary block):
...
byte-identical to :mod:`wakir_protocol.identity_substrate.aip_signing`. The ``kid``
...
from wakir_protocol.identity_substrate import _jcs_pure
from wakir_protocol.schemas.registry_nats_kv_backend import (
    SchemaRegistryEntry,
...
# ---------------------------------------------------------------------------
# JCS resolver indirection (mirrors wakir_protocol.identity_substrate.aip_signing).
# ---------------------------------------------------------------------------
...
def _jcs_canonicalize(value: object) -> bytes:
    ...
    canonicaliser in :mod:`wakir_protocol.identity_substrate._jcs_pure`. The two paths
...
def _entry_to_signing_payload(
    ...
    Mirrors :func:`wakir_protocol.schemas.registry_nats_kv_backend._entry_to_envelope`
    field-for-field so the JCS pre-image is byte-identical to the
This module envelope minus signature.
    """
...
def _canonical_signing_payload(entry: SchemaRegistryEntry) -> bytes:
    ...
    convention (`wakir_protocol.identity_substrate.aip_signing._canonical_signing_payload`).
    """
...
# This module codec would already have raised; this is defence-in-depth.
        raise SchemaRegistryEnvelopeError(
            f"envelope re-parse failed: {exc!r}"
        ) from exc
    if not isinstance(payload, dict):  # pragma: no cover -- this module catches
        raise SchemaRegistryEnvelopeError(
