...
This module (S3-1) lands the production-target backend
...
the DevOps track's Phase-1 bucket inventory (``scripts/init-nats-buckets.py``,
This module).
...
*consumer* of the DevOps track's existing :data:`PHASE_1_BUCKETS[0]` slot. The
This module V-908 backend added ``wakir-federation-routes`` as the 5th
bucket; This module introduces no 6th bucket.

Design choices
--------------

The backend mirrors the V-908 NATS-KV route-registry pattern shipped
in (``route_registry_nats_kv_backend.py``).
Specifically:
...
name detection) was already present in the V-908 this module backend
pattern; This module lands the schema-registry side of
...
This module): replication-aware quorum upserts, deprecation policy with
...
The pattern mirrors the V-908 this module watch-stream-snapshot layer
...
and mirror the V-908 this module ``test_..._watch_stream`` pattern with a
...
- V-908 this module watch-stream pattern source: same module, ``WatchOp`` /
...
#: the DevOps track's :data:`PHASE_1_BUCKETS[0]` (``scripts/init-nats-buckets.py``).
...
def _jcs_canonicalise(obj: Any) -> bytes:
    ...
    Prefers ``wakir_protocol.canonical.jcs.canonicalise`` when importable;
    ...
    """
    try:
        from wakir_protocol.canonical.jcs import canonicalise  # type: ignore
...
# ---------------------------------------------------------------------------
#
# This module (S3-1) shipped get/put/delete/snapshot. Snapshot is full-bucket:
...
#
# This module (S3-4) adds a watch-based incremental layer that consumes
...
This module (S3-4) substrate. Initialises an in-memory
