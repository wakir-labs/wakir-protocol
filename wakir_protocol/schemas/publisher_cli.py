...
from wakir_protocol.schemas.entry_signing import (
...
def _add_capability_flags(p: argparse.ArgumentParser) -> None:
    ...
    - ``--sign`` plus a private-key source (``--ed25519-priv-key-hex``
      ...
      via :func:`wakir_protocol.schemas.entry_signing.sign_entry`.

    - ``--gate`` enables gating via
      :func:`wakir_protocol.schemas.registered_by_capability.gate_signed_entry`.
...
async def _default_connect_factory(
    ...
    wakir_protocol.schemas.publisher_cli`` against a live cluster gets a
    working path.
    """
