...
Scope: skeleton + MOCK functional implementation
for ``fetch_jwt_svid``; ``fetch_x509_svid`` remains stub.
Status: surface contract, type annotations, docstrings, mock impl. Real
upstream-``spiffe``-backed implementation deferred to
Phase-2c (paired with the DevOps-track SPIRE-server integration on the DevOps track's
track).

This module additive note: :class:`MockSpiffeWorkloadApiAdapter` is
...
the concrete implementation slot until / Phase-2c lands
it.
...
- DevOps track items (this module / Phase-2c):
...
class SpiffeWorkloadApiAdapter(Protocol):
    ...
    Concrete implementation deferred to / Phase-2c. The
