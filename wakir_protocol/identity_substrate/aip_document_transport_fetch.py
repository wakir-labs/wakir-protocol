...
"""This module AIP-document transport-fetch.

This module is the Phase-2 follow-up to this module
...
2.  Fetches the body via the Phase-1b V-908 HTTPS-transport layer
    (``wakir_protocol.identity_substrate.aip_https_backend.HTTPSDocumentTransport``).
...
reuse :func:`wakir_protocol.identity_substrate.dns_anchor.parse_anchor` verbatim.

V-908 spec cross-references
---------------------------

* V-908 §3.3 (HTTPS transport) — fully delegated to this module
...
* Schema-Registry spec §5.10 (this module follow-up to
...
This module closes the Zone-1-annex follow-up slot
"AIP-document transport-fetch" without touching any of the four
Z-1-K consensus points:

* Z-1-K-1 (kid-Resolver-Shape) — non-touched; An earlier increment closed it.
This module feeds the resolver, does not modify its surface.
...
This module layer does NOT wrap them so callers retain typed access to
...
This module accepts two shapes:
...
class AipFetchResult:
    """The outcome of a successful this module AIP-document fetch.
...
def _jcs_anchor_hex(aip_doc: dict) -> str:
    ...
    :mod:`wakir_protocol.identity_substrate.aip_signing`; we import it lazily to avoid
    a circular module dependency at import time and to keep this module
...
def fetch_aip_document(
    ...
    *,
    ...
    HTTPS-transport layer (this module ``HTTPSDocumentTransport``) with the
