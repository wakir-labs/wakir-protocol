...
"""This module AIP-document signature-verification cache tier.

This module ships a stateful caching tier on top of this module
...
* Cache transport-fetch outputs. :class:`AipFetchResult` byte-anchors
  (``jcs_sha256_hex``) are produced by this module
...
* Z-1-K-2 (JCS-Resolver-Lock) — non-touched; the cache
  ...
  byte-identical (the cache-key construction re-uses this module
...
* Z-1-K-4 (STRICT-Mode-Activation-Owner) — non-touched;
  ...
  (this module ``VerifyMode.STRICT``) is upstream and unchanged.
...
def _jcs_body_digest_hex(aip_doc: dict) -> str:
    ...
    Byte-identical to :func:`wakir_protocol.identity_substrate.aip_document_transport_fetch._jcs_anchor_hex`
