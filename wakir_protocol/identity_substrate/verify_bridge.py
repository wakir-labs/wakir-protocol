...
layer (this module :class:`~wakir_protocol.identity_substrate.aip_https_backend.HTTPSDocumentTransport`),
the AIP-document signature helpers (:mod:`wakir_protocol.identity_substrate.aip_signing`),
and the JCS / schema validation indirections introduced by this module.
...
1. **Result-type, not exceptions.** Federation resolvers iterate over
   ...
   try/except scaffolding. This module :class:`FederationResolver` and
...
- ETag / Cache-Control honouring (the bridge calls ``transport.get``
  ...
  responsibility, mirroring the earlier increment's separation of concerns).
...
- This module outbox spec (wirelang-eng workspace, 2026-05-06).
- This module outbox spec (wirelang-eng workspace, 2026-05-07).
    ...
    *,
    ...
    Args:
        ...
        signature_verifier: signature-verify callable; default is
            :func:`wakir_protocol.identity_substrate.aip_signing.verify_aip_signature`.
...
def _default_schema_loader(schema_name: str) -> dict:
    ...
    if schema_name == "aip-document/0.1.0":
        ...
        schema_text = (
            resources.files("wakir_protocol.schemas").joinpath("aip-document.json").read_text(
                encoding="utf-8"
            )
        )
...
def make_https_aip_verify_fn(
    *,
    ...
    :class:`wakir_protocol.identity_substrate.aip_https_backend.HTTPSAipResolver`.
    ...
    returns a closure that runs the full this module verify pipeline against
...
This module the simple "first entry" convention matches the existing
    Phase-1a generator.
    """
