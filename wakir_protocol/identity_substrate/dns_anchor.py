...
# This module risk-item §8.2 ("TTL-spoofing-risiko bei niedrigen Cache-TTLs"):
...
# V-908 spec §3.4 for the spec-level wording added in (B-Mikro).
...
class TxtResolver(Protocol):
    ...
    rule (this module minor edit; see §3.4 of the spec for canonical wording).
...
This module verification (2026-05-06): both providers return RFC-8484
    JSON with HTTP 200 and ``Status: 0`` for live ``example.com``
    TXT lookups in the project sandbox. This module outbox §1 / §7.
    ...
    def _extract_txt_strings(self, answers: list[dict]) -> Iterable[str]:
        ...
        in the ``data`` field. V-908 §3.4 (this module minor edit) requires
...
class DnsPythonResolver:
    ...
    Future-default note (this module outbox §1.2): once the
    ...
    equivalence cross-check (this module outbox §2.3 test category 3).
